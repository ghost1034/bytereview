...
To get a first-pass layout, click **Place fields with AI**. Analyze every document or only the active document, and optionally describe the layout you want. Your current manual edits save first, then analysis continues in the background; you can leave the editor and return to the latest run. The selected pages count toward your page allowance when analysis completes, even if you later discard the suggestions. When you send an envelope, every page across its documents is also recorded as E-Signature page usage, whether fields were placed manually or with AI.

AI suggestions appear as dashed, non-editable overlays and never replace or autosave with your existing fields. Review the signing role, field type, page, and any warnings. Remove unwanted suggestions, then choose **Apply remaining** to add the rest or **Discard all**. Ambiguous anchors, signing-role assignments, unsupported field types, invalid geometry, and suggestions that substantially overlap existing fields are omitted with an explanation. Apply or discard a completed run before review and sending can finish. AI can make mistakes, so use manual placement or **Place by anchor** whenever the suggested layout is incomplete.
