#!/usr/bin/env bash
# AccountingClaw desktop installer (macOS / Linux).
#
# Installs the AccountingClaw skills bundle into your local Hermes Agent home
...
#   # or download and run:
#   bash install-accountingclaw.sh [--force] [cpaa_live_...]
#
...
MARKER_FILE="$HERMES_HOME/.cpaa/accountingclaw-installed"
...
if [ -z "$KEY" ]; then
  if [ -t 0 ]; then
    read -r -p "Enter your AccountingClaw activation key (cpaa_live_...): " KEY
  fi
fi
if [ -z "$KEY" ]; then
  echo "Activation key required."
  echo "Get yours at https://cpaautomation.ai/dashboard/activation, then run:"
  echo "  CPAA_ACTIVATION_KEY=\"cpaa_live_...\" bash install-accountingclaw.sh"
  exit 64
fi
...
payload="{\"activation_key\":\"$KEY\",\"fingerprint\":\"$fingerprint\",\"install_type\":\"desktop\"}"
...
echo "Downloading the AccountingClaw bundle ..."
curl -fsSL "$bundle_url" -o "$tmp_dir/accountingclaw-profile.tar.gz"

if [ -n "$expected_sha" ]; then
  if command -v sha256sum >/dev/null 2>&1; then
    actual_sha="$(sha256sum "$tmp_dir/accountingclaw-profile.tar.gz" | awk '{print $1}')"
  else
    actual_sha="$(shasum -a 256 "$tmp_dir/accountingclaw-profile.tar.gz" | awk '{print $1}')"
  fi
...
echo
echo "AccountingClaw installed into $HERMES_HOME."
echo "Next steps:"
echo "  1. Launch Hermes Desktop (or run: hermes desktop)."
echo "  2. Open the Skills pane — the AccountingClaw skills are ready to use."
