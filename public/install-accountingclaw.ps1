# AccountingClaw desktop installer (Windows 10/11).
#
...
#   # or download and run:
#   .\install-accountingclaw.ps1 -Key cpaa_live_... [-Force]
#
...
$MarkerFile = Join-Path $HermesHome ".cpaa\accountingclaw-installed"
...
if (-not $Key) { $Key = Read-Host "Enter your AccountingClaw activation key (cpaa_live_...)" }
if (-not $Key) {
...
try {
    ...
    Write-Host "Downloading the AccountingClaw bundle ..."
    $BundlePath = Join-Path $TmpDir "accountingclaw-profile.tar.gz"
    ...
    Write-Host ""
    Write-Host "AccountingClaw installed into $HermesHome."
    Write-Host "Next steps:"
    Write-Host "  1. Launch Hermes Desktop."
    Write-Host "  2. Open the Skills pane - the AccountingClaw skills are ready to use."
