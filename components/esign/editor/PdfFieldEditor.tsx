...
function PropertiesPanel({ field, fields, update, updateRadioGroup, remove }: {
  ...
  const canAlignText = supportsTextAlignment(field.fieldType)
  ...
  return <div className="space-y-2 rounded-md border border-border bg-surface p-3 text-sm">
    ...
    {canAlignText && <details>
      ...
      <div className="mt-2 space-y-2">
        ...
        <label className="block"><span className="mb-1 block text-xs font-medium">Text alignment</span><select aria-label="Text alignment" className="w-full rounded border border-border bg-background px-2 py-1" value={properties.appearance?.alignment ?? 'left'} onChange={(event) => setProperties({ appearance: { ...properties.appearance, alignment: event.target.value as 'left' | 'center' | 'right' } })}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label>
        ...
        <PdfPageCanvas pdf={pdf} pageNumber={pageIndex + 1} overlay={(size) => <div className="absolute inset-0" onPointerMove={(event) => {
          ...
          {fields.filter((field) => field.documentId === activeDocument.id && field.pageNumber === pageIndex).map((field) => { const color = participantColor(participantIndexById.get(field.participantId) ?? 0); const selected = selectedIds.has(field.id); const isConditionalParent = selectedField?.properties?.conditional?.parent_field_id === field.id
            return <div key={field.id} id={`esign-editor-field-${field.id}`} onPointerDown={(event) => startInteraction(event, field, 'move', size)} onPointerMove={moveInteraction} onPointerUp={endInteraction}
              ...
              style={{ left: field.posX * size.width, top: field.posY * size.height, width: field.width * size.width, height: field.height * size.height, borderColor: color.border, backgroundColor: color.bg, color: color.text, cursor: 'move', justifyContent: supportsTextAlignment(field.fieldType) ? ({ left: 'flex-start', center: 'center', right: 'flex-end' } as const)[field.properties?.appearance?.alignment ?? 'left'] : undefined, textAlign: field.properties?.appearance?.alignment }}>
