...
function Matches({ matches, flush }: { matches: ConflictMatch[]; flush?: boolean }) {
  if (!matches.length) return <div className="text-[12px] leading-4 text-crm-success-700">No matches found.</div>;
  return (
    <div className={flush ? "-mx-5 border-t border-crm-sand-150 [&:not(:last-child)]:border-b" : "overflow-hidden rounded-crm-lg border border-crm-sand-150"}>
        ...
        <tbody>{matches.map((m, i) => (
          <tr key={i}>
            ...
            <td className="!h-auto !py-2 align-top">
              <div className="font-medium text-crm-sand-900">{m.matched_name}</div>
              {m.context && <div className="text-[12px] leading-4 text-crm-sand-500">{m.context}</div>}
              {m.restricted && <div className="mt-0.5 inline-flex items-center gap-1 text-[12px] leading-4 text-crm-sand-500"><Lock size={12} />Restricted matter — details withheld by an ethical wall</div>}
            </td>
...
function ResolveModal({ check, onClose }: { check: ConflictCheck; onClose: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={<>{override ? "Override decision" : `Resolve ${check.check_type} check`} <span className="mono text-crm-sand-500">#{check.id}</span></>} size="wide"
      ...
      <div className="space-y-4">
        ...
        {check.independence_attestation && (
          <div>
            ...
            <ul className="divide-y divide-crm-sand-100 rounded-crm-md border border-crm-sand-150">
              {INDEPENDENCE_QUESTIONS.map((q) => { const yes = !!check.independence_attestation?.[q.key]; return (
                <li key={q.key} className="flex items-start gap-3 px-3 py-2 text-[13px] leading-5"><Badge dot tone={yes ? "danger" : "success"} className="mt-0.5 w-10 shrink-0">{yes ? "Yes" : "No"}</Badge><span className={yes ? "text-crm-sand-900" : "text-crm-sand-600"}>{q.label}</span></li>); })}
            </ul>
          </div>)}
        {readOnly ? (
          <div className="flex items-start gap-2 rounded-crm-md border border-crm-warn-200 bg-crm-warn-50 px-3 py-2.5 text-[13px] leading-5 text-crm-warn-700" role="status">
            <Lock size={14} className="mt-0.5 shrink-0 text-crm-warn-600" aria-hidden />
          ...
          {override && (
            <div className="flex items-start gap-2 rounded-crm-md border border-crm-warn-200 bg-crm-warn-50 px-3 py-2.5 text-[12px] leading-4 text-crm-warn-700" role="status">
          ...
          {override && check.resolution_note && <div className="rounded-crm-md bg-crm-sand-25 px-3 py-2 text-[13px] leading-5 text-crm-sand-700"><span className="text-crm-sand-500">Prior note:</span> {check.resolution_note}</div>}
...
export function ClearanceList({ checks, loading, compact }: { checks: ConflictCheck[] | undefined; loading?: boolean; compact?: boolean }) {
  ...
  if (!checks?.length) return compact ? <div className="text-[12px] leading-4 text-crm-sand-500">No checks recorded.</div> : <Empty title="No clearance checks" hint="Run a check from an opportunity or account." />;
  return (
    <div className={compact ? "divide-y divide-crm-sand-100" : "space-y-3"}>
      {checks.map((c) => {
        ...
        return (
          <article key={c.id} className={compact ? "py-3 text-[12px] leading-4 first:pt-0 last:pb-0" : "card px-5 py-4 text-[13px] leading-5"}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                  <span className="font-medium text-crm-sand-900">{titleCase(c.check_type)} check</span>
                  <span className="mono text-crm-sand-500">#{c.id}</span>
                  <Badge dot tone={statusTone(c.status)}>{titleCase(c.status)}</Badge>
                  {c.status === "waived" && <ShieldOff size={12} className="text-crm-success-600" aria-label="Waived" />}
                </div>
                <div className="mt-0.5 text-[12px] leading-4 text-crm-sand-500">
                  {c.requested_by_name}<span className="mx-1.5 text-crm-sand-300">·</span><span className="num">{fmtDateTime(c.created_at)}</span>
                  {!compact && c.opportunity_id && <><span className="mx-1.5 text-crm-sand-300">·</span><Link to={`/opportunities/${c.opportunity_id}`} className="text-crm-sand-700">{c.opportunity_name}</Link></>}
                  {!compact && !c.opportunity_id && c.account_id && <><span className="mx-1.5 text-crm-sand-300">·</span><Link to={`/accounts/${c.account_id}`} className="text-crm-sand-700">{c.account_name}</Link></>}
                </div>
              </div>
            ...
            <div className={cn("text-crm-sand-600", compact ? "mt-1.5" : "mt-2.5")}><span className="text-crm-sand-500">Parties:</span> {c.parties.join("; ")}</div>
            {c.matches.length > 0 && (compact
              ? <div className="mt-1.5 text-crm-warn-700"><span className="num">{c.matches.length}</span> potential match{c.matches.length === 1 ? "" : "es"}: {c.matches.slice(0, 3).map((m) => m.matched_name).join(", ")}{c.matches.length > 3 ? "…" : ""}</div>
              : <div className="mt-3"><Matches matches={c.matches} flush /></div>)}
            {disclosed.length > 0 && <div className="mt-1.5 text-crm-warn-700">Attestation disclosed: {disclosed.map((q) => q.key.replace(/_/g, " ")).join(", ")}</div>}
            {c.resolution_note && <div className={cn("rounded-crm-md bg-crm-sand-25 px-3 py-2 text-crm-sand-700", compact ? "mt-2" : "mt-3")}><span className="text-crm-sand-500">Resolution ({c.resolved_by_name}, <span className="num">{fmtDateTime(c.resolved_at)}</span>):</span> {c.resolution_note}</div>}
          </article>);
      })}
          ...
          <Card title="Search the firm database">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="mt-3 text-[12px] leading-4 text-crm-sand-500">Ad-hoc search does not record a check. Run the check from the opportunity to create an auditable record.</div>
              </div>
              <div>
                <div className="label">Results</div>
                {results === null ? <div className="text-[12px] leading-4 text-crm-sand-500">Results will appear here.</div> : <Matches matches={results} />}
              </div>
            </div>
          </Card>)}
      </div>
