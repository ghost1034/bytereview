...
import { reportsApi } from "@/components/firmcrm/api";
import { Card, Empty, PageHeader, Stat, Tabs, cn } from "@/components/firmcrm/components/ui";
import { CHART, ChartFrame, ChartLegend, barChartProps, barProps, gridProps, tooltipProps, xAxisProps, } from "@/components/firmcrm/components/crm/charts";
import { useMoney, num, pct, titleCase } from "@/components/firmcrm/lib/format";
        ...
        {rows.map((r, i) => (
          <tr key={i}>
            {cols.map((c, j) => {
              ...
              return (
                <td key={c.k} className={cn(c.right && "text-right num whitespace-nowrap", c.money && !dim && "font-medium", j === 0 && !c.right && "font-medium", dim && "text-crm-sand-300")}>
                  {dim ? "—" : text}
                </td>
              );
            })}
...
function KpiRow({ children }: { children: ReactNode }) { return <div className="grid grid-cols-2 lg:grid-cols-5 items-stretch gap-4">{children}</div>; }
    ...
    <div>
      ...
      <div className="mt-5 space-y-4">
        {tab === "pipeline" && (<>
          <Card title="Open pipeline by stage" padded={false}>
            <Table
              ...
              totals={pipe.data ? { count: num(pipe.data.total_count), amount: money(pipe.data.total_amount), weighted: money(pipe.data.total_weighted), stale: pipe.data.stale_count ? num(pipe.data.stale_count) : <span className="text-crm-sand-300">—</span> } : undefined}
            />
          </Card>
          <Card title="Stage velocity" actions={<span className="text-crm-sand-500">Average days in each stage before advancing</span>} padded={false}>
        ...
        {tab === "winloss" && (!wl.data ? <Loading /> : (<>
          ...
          <Card title="Monthly closed" actions={<><ChartLegend items={[{ label: "Won", color: CHART.success }, { label: "Lost", color: CHART.dangerSoft }]} /><span className="ml-3 text-crm-sand-500">Trailing 12 months</span></>}>
        ...
        {tab === "origination" && (
          <Card title="Origination credit by partner" padded={false}>
            <div className="border-b border-crm-sand-150 px-5 py-3 text-[12px] leading-4 text-crm-sand-500">
        ...
        {tab === "referrals" && (
          <Card title="Referral sources" actions={<span className="text-crm-sand-500">Who sends us work</span>} padded={false}>
        ...
        {tab === "funnel" && (!fun.data ? <Loading /> : (<>
          ...
          <Card title="Lead sources" actions={<span className="text-crm-sand-500">Trailing 12 months</span>} padded={false}>
        ...
        {tab === "activity" && (
          <Card title="Activity leaderboard" actions={<span className="text-crm-sand-500">Last 30 days</span>} padded={false}>
