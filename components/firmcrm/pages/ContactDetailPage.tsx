...
import { Link, useParams } from "@/components/firmcrm/lib/navigation";
import { Archive, ArchiveRestore, Pencil } from "lucide-react";
import { useAuth } from "@/components/firmcrm/lib/auth";
import { contactsApi, oppsApi } from "@/components/firmcrm/api";
import type { Contact } from "@/components/firmcrm/api/types";
import { Badge, Button, Card, DL, Empty, PageHeader, Spinner, statusTone } from "@/components/firmcrm/components/ui";
...
import { Dash, cellMoney } from "@/components/firmcrm/components/ui/cells";
...
export default function ContactDetailPage() {
  ...
  const facts: Fact[] = [
    { label: "Account", value: k.account_name ? <Link to={`/accounts/${k.account_id}`} className="text-crm-sand-900 hover:text-crm-accent-600">{k.account_name}</Link> : <Dash />, sub: [k.title, k.role ? titleCase(k.role) : null].filter(Boolean).join(" · ") || "No title", size: "text" },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 items-start gap-4">
        <Card title="Activity" className="lg:col-span-8"><ActivityTimeline filter={{ contact_id: id }} linkTo /></Card>
          ...
          <Card title="Related opportunities" padded={false}>
            {referred.isLoading ? <Spinner /> : !related.length ? <Empty title="No related opportunities" hint="Opportunities where this person is the primary or referring contact appear here." /> : (
              <table className="tbl"><tbody>{related.map((o) => (
                <tr key={o.id}>
                  <td><Link to={`/opportunities/${o.id}`} className="font-medium">{o.name}</Link><div className="text-[12px] leading-4 text-crm-sand-500">{o.referral_contact_id === id ? "Referred" : "Primary contact"} · {o.stage_name}</div></td>
