...
import { fmtDate, useMoney } from "@/components/firmcrm/lib/format";

/** Em dash in the disabled tone: empty numerics, blank dates, blank facts. */
export const Dash = () => <span className="text-crm-sand-300">—</span>;
/** Money cell. DataTable's `align: "right"` already applies tabular 500; zero/empty renders a dash. */
function MoneyValue({ n }: { n: number | null | undefined }) {
  const money = useMoney();
  return <>{n ? money(n) : <Dash />}</>;
}
export const cellMoney = (n: number | null | undefined): ReactNode => <MoneyValue n={n} />;
...
export function NameCell({ name, sub, chips, max = 320, title }: { name: ReactNode; sub?: ReactNode; chips?: ReactNode; max?: number; title?: string }) {
  return (
    <div className="min-w-0" style={{ maxWidth: max }}>
      <div className="flex items-center gap-2 font-medium text-crm-sand-900"><span className="truncate" title={title ?? (typeof name === "string" ? name : undefined)}>{name}</span>{chips}</div>
      {sub ? <div className="truncate text-[12px] leading-4 text-crm-sand-500">{sub}</div> : null}
    </div>
  );
}
...
export function SearchInput({ className, ...p }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div className={cn("relative", className ?? "w-[280px]")}>
      <Search size={14} className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-crm-sand-400" aria-hidden />
...
export function FilterToggle({ checked, onChange, children }: { checked: boolean; onChange: (v: boolean) => void; children: ReactNode }) {
  return (
    <label className="flex items-center gap-1.5 text-[12px] leading-4 text-crm-sand-600 select-none">
...
export const ResultCount = ({ children }: { children: ReactNode }) => <span className="num text-[12px] leading-4 text-crm-sand-500">{children}</span>;
