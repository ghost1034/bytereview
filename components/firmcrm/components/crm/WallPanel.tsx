...
import { wallsApi } from "@/components/firmcrm/api";
import { ApiError } from "@/components/firmcrm/api/client";
import { Badge, Button, Card, Field, Modal, Select, Textarea } from "@/components/firmcrm/components/ui";
...
import { useUsers, opt } from "@/components/firmcrm/lib/hooks";
import { fmtDateTime, initials } from "@/components/firmcrm/lib/format";
...
export function WallPanel({ entityType, id, entityName }: { entityType: "account" | "opportunity"; id: number; entityName: string }) {
  ...
  const [creating, setCreating] = useState(false); const [reason, setReason] = useState(""); const [members, setMembers] = useState<string[]>([]); const [adding, setAdding] = useState("");
  ...
  const add = useMutation({ mutationFn: (uid: string) => wallsApi.addMember(wall.data!.id, uid), onSuccess: () => { inv(); setAdding(""); }, onError: error });
  const remove = useMutation({ mutationFn: (uid: string) => wallsApi.removeMember(wall.data!.id, uid), onSuccess: inv,
  ...
  const canRemove = (uid: string) => uid !== user?.id || otherPrivileged;
  return (
    <Card tourId="wall-panel" title={<span className="inline-flex items-center gap-1.5">{w ? <Lock size={14} className="text-crm-danger-600" /> : <LockOpen size={14} className="text-crm-sand-500" />}Access{w && <Badge tone="danger">Restricted</Badge>}</span>}
      ...
      {!w ? <div className="text-[12px] leading-4 text-crm-sand-500">Visible firm-wide. Raise an ethical wall to restrict this {entityType} and everything under it to a named team.</div> : (
        <div className="space-y-3 text-[13px] leading-5">
          <div className="text-crm-sand-900">{w.reason}</div>
          <div className="text-[12px] leading-4 text-crm-sand-500">Raised by {w.created_by_name} · <span className="num">{fmtDateTime(w.created_at)}</span>. Non-members cannot see this record; conflict searches show it as a restricted matter.</div>
            ...
            {w.members.map((m) => (
              <li key={m.user_id} className="flex h-9 items-center justify-between gap-2 px-3">
                <span className="inline-flex min-w-0 items-center gap-2">
                  <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-crm-sand-200 text-[9px] font-semibold text-crm-sand-700">{initials(m.full_name)}</span>
                  <span className="truncate">{m.full_name}</span><span className="text-[12px] text-crm-sand-500">{m.role}</span>
      ...
      <Modal open={creating} onClose={() => setCreating(false)} title="Raise ethical wall" footer={<><Button onClick={() => setCreating(false)}>Cancel</Button><Button variant="primary" disabled={reason.trim().length < 5 || create.isPending} onClick={() => create.mutate()}>Raise wall</Button></>}>
        <div className="space-y-4">
          <p className="text-[13px] leading-5 text-crm-sand-600">Restricts <span className="font-medium text-crm-sand-900">{entityName}</span> and everything under it to the team named below.</p>
          ...
          <Field label="Team with access" hint="You are always included. Admins can see walled records by default; ask your administrator if that should change.">
            <div className="grid max-h-[200px] grid-cols-2 gap-1.5 overflow-auto rounded-crm-md border border-crm-sand-150 p-3">
              {users.data?.filter((u) => u.id !== user?.id).map((u) => <label key={u.id} className="flex items-center gap-2 text-[13px] leading-5 text-crm-sand-900"><input type="checkbox" checked={members.includes(u.id)} onChange={(e) => setMembers(e.target.checked ? [...members, u.id] : members.filter((x) => x !== u.id))} />{u.full_name} <span className="text-[12px] text-crm-sand-500">{u.role}</span></label>)}
            </div>
          </Field>
        </div>
      </Modal>
