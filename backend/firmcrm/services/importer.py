...
from firmcrm.models import Account, Contact, ImportJob, Lead, PracticeArea, User
from firmcrm.schemas import FirmCrmAccountCreate, FirmCrmContactCreate, FirmCrmLeadCreate
...
def _import_account(db: Session, res: _Resolver, row: dict, i: int, rep: Report, seen: set[str]) -> None:
    ...
    seen.add(key)
    payload = _blank_to_none({k: row.get(k) for k in FirmCrmAccountCreate.model_fields if k in row})
    ...
    data = FirmCrmAccountCreate(**{k: v for k, v in payload.items() if v is not None or k in ("owner_id", "originating_partner_id")})
...
def _import_contact(db: Session, res: _Resolver, row: dict, i: int, rep: Report, seen: set[str]) -> None:
    ...
    if email:
        ...
        seen.add(email)
    payload = _blank_to_none({k: row.get(k) for k in FirmCrmContactCreate.model_fields if k in row})
    payload["email"] = email
    ...
    if rep.exceptions and rep.exceptions[-1]["row"] == i:
        raise DomainError("unresolved references")
    data = FirmCrmContactCreate(**{k: v for k, v in payload.items() if v is not None or k in ("account_id", "owner_id")})
...
def _import_lead(db: Session, res: _Resolver, row: dict, i: int, rep: Report, seen: set[str]) -> None:
    ...
    seen.add(key)
    payload = _blank_to_none({k: row.get(k) for k in FirmCrmLeadCreate.model_fields if k in row})
    payload["email"] = email
    payload["practice_area_id"] = res.practice_area(row.get("practice_area"), rep, i, row)
    payload["owner_id"] = res.user(row.get("owner_email"), rep, i, "owner_email", row)
    if rep.exceptions and rep.exceptions[-1]["row"] == i:
        raise DomainError("unresolved references")
    data = FirmCrmLeadCreate(**{k: v for k, v in payload.items() if v is not None or k in ("practice_area_id", "owner_id")})
    existing = None
    if email:
