...
from firmcrm.models import Account, Contact, Engagement, Opportunity
...
def _search(db: Session, parties: list[str]) -> list[dict]:
    threshold = settings_for(db).conflict_match_threshold
...
def latest_status_for_opportunity(db: Session, opportunity_id: int, check_type: str | None = None) -> str | None:
    from firmcrm.models import ConflictCheck

    row = db.scalars(
        select(ConflictCheck).where(ConflictCheck.opportunity_id == opportunity_id, ConflictCheck.check_type == check_type).order_by(ConflictCheck.id.desc())
    ).first()
    if row and check_type == "independence" and row.independence_attestation is None:
        return "pending"
    return row.status if row else None


def search(db, parties):
    with db.include_restricted():
        return _search(db, parties)
