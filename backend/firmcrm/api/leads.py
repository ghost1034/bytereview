...
from firmcrm.enums import LEAD_STATUSES_ALL, LeadSource
from firmcrm.models import Lead, User
from firmcrm.schemas import FirmCrmLeadConvertIn, FirmCrmLeadConvertOut, FirmCrmLeadCreate, FirmCrmLeadOut, FirmCrmLeadUpdate, FirmCrmPage
from firmcrm.services import leads as lead_svc

router = APIRouter(route_class=FirmCrmRoute, prefix="/leads", tags=["leads"])


def _enrich(db: Session, rows: list[Lead]) -> list[FirmCrmLeadOut]:
    un = user_names(db, [l.owner_id for l in rows])
    pn = practice_area_names(db, [l.practice_area_id for l in rows])
    out = []
    for l in rows:
        o = FirmCrmLeadOut.model_validate(l)
...
@router.get("", response_model=FirmCrmPage[FirmCrmLeadOut])
def list_leads(q: str | None = Query(None, max_length=200), status: str | None = Query(None, max_length=20),
               source: LeadSource | None = None, owner_id: str | None = None, include_converted: bool = False,
...
@router.post("", response_model=FirmCrmLeadOut, status_code=201)
def create_lead(body: FirmCrmLeadCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.get("/{lead_id}", response_model=FirmCrmLeadOut)
...
@router.patch("/{lead_id}", response_model=FirmCrmLeadOut)
def update_lead(lead_id: int, body: FirmCrmLeadUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.post("/{lead_id}/convert", response_model=FirmCrmLeadConvertOut)
def convert_lead(lead_id: int, body: FirmCrmLeadConvertIn, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
    l = get_or_404(db, Lead, lead_id)
    account, contact, opp = lead_svc.convert(db, l, body, actor)
    db.commit()
    return FirmCrmLeadConvertOut(account_id=account.id, contact_id=contact.id, opportunity_id=opp.id if opp else None)


@router.post("/{lead_id}/archive", response_model=FirmCrmLeadOut)
...
@router.post("/{lead_id}/restore", response_model=FirmCrmLeadOut)
