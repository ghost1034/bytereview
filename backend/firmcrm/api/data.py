...
from firmcrm.models import ImportJob, User
from firmcrm.schemas import FirmCrmPage
from firmcrm.services import exporter, importer

router = APIRouter(route_class=FirmCrmRoute, tags=["data"])
...
class FirmCrmImportException(BaseModel):
    row: int
    field: str | None
    message: str
    data: dict[str, str]
...
class FirmCrmImportJobOut(BaseModel):
    id: int
    entity: str
    filename: str
    dry_run: bool
    ...
    updated_rows: int
    skipped_rows: int
    exceptions: list[FirmCrmImportException]
    actor_id: str | None
    actor_name: str | None = None
    created_at: str


def _out(db: Session, jobs: list[ImportJob]) -> list[FirmCrmImportJobOut]:
...
@router.post("/import/{entity}", response_model=FirmCrmImportJobOut)
async def import_csv(entity: ImportEntity, file: UploadFile = File(...), dry_run: bool = Form(True),
    ...
    raw = await file.read(importer.MAX_BYTES + 1)
    job = importer.run(db, entity, file.filename or "upload.csv", raw, dry_run=dry_run, actor=actor)
    return _out(db, [job])[0]


@router.get("/import/jobs", response_model=FirmCrmPage[FirmCrmImportJobOut])
