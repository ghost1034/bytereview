...
from firmcrm.enums import ContactRole, Lifecycle
from firmcrm.models import Contact, User
from firmcrm.schemas import FirmCrmContactCreate, FirmCrmContactOut, FirmCrmContactUpdate, FirmCrmPage
from firmcrm.services import visibility

router = APIRouter(route_class=FirmCrmRoute, prefix="/contacts", tags=["contacts"])


def _enrich(db: Session, rows: list[Contact]) -> list[FirmCrmContactOut]:
    an = account_names(db, [c.account_id for c in rows])
    un = user_names(db, [c.owner_id for c in rows])
    out = []
    for c in rows:
        o = FirmCrmContactOut.model_validate(c)
...
@router.get("", response_model=FirmCrmPage[FirmCrmContactOut])
def list_contacts(q: str | None = Query(None, max_length=200), account_id: int | None = None, lifecycle: Lifecycle | None = None,
                  role: ContactRole | None = None, owner_id: str | None = None, include_archived: bool = False,
...
@router.post("", response_model=FirmCrmContactOut, status_code=201)
def create_contact(body: FirmCrmContactCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.get("/{contact_id}", response_model=FirmCrmContactOut)
...
@router.patch("/{contact_id}", response_model=FirmCrmContactOut)
def update_contact(contact_id: int, body: FirmCrmContactUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.post("/{contact_id}/archive", response_model=FirmCrmContactOut)
...
@router.post("/{contact_id}/restore", response_model=FirmCrmContactOut)
