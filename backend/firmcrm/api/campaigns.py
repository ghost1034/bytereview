...
from firmcrm.api.common import SortDir, apply_sort, apply_updates, archive, get_or_404, paginate, restore
...
from firmcrm.core.errors import Conflict
from firmcrm.enums import CampaignStatus
from firmcrm.models import Campaign, CampaignMember, Contact, Lead, Opportunity, User
from firmcrm.schemas import FirmCrmCampaignCreate, FirmCrmCampaignMemberIn, FirmCrmCampaignMemberOut, FirmCrmCampaignOut, FirmCrmCampaignUpdate, FirmCrmPage

router = APIRouter(route_class=FirmCrmRoute, prefix="/campaigns", tags=["campaigns"])


def _enrich(db: Session, rows: list[Campaign]) -> list[FirmCrmCampaignOut]:
    ...
    out = []
    for c in rows:
        d = FirmCrmCampaignOut.model_validate(c)
...
@router.get("", response_model=FirmCrmPage[FirmCrmCampaignOut])
...
@router.post("/{campaign_id}/archive", response_model=FirmCrmCampaignOut)
...
@router.post("/{campaign_id}/restore", response_model=FirmCrmCampaignOut)
...
@router.post("", response_model=FirmCrmCampaignOut, status_code=201)
def create_campaign(body: FirmCrmCampaignCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.get("/{campaign_id}", response_model=FirmCrmCampaignOut)
...
@router.patch("/{campaign_id}", response_model=FirmCrmCampaignOut)
def update_campaign(campaign_id: int, body: FirmCrmCampaignUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
    c = get_or_404(db, Campaign, campaign_id)
...
@router.get("/{campaign_id}/members", response_model=list[FirmCrmCampaignMemberOut])
def list_members(campaign_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    out = []
    for m in rows:
        d = FirmCrmCampaignMemberOut.model_validate(m)
...
@router.post("/{campaign_id}/members", response_model=FirmCrmCampaignMemberOut, status_code=201)
def add_member(campaign_id: int, body: FirmCrmCampaignMemberIn, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
    get_or_404(db, Campaign, campaign_id)
...
@router.patch("/{campaign_id}/members/{member_id}", response_model=FirmCrmCampaignMemberOut)
def update_member(campaign_id: int, member_id: int, body: FirmCrmCampaignMemberIn, db: Session = Depends(get_db),
