...
from firmcrm.enums import ActivityKind
from firmcrm.models import Activity, Contact, Opportunity, User, utcnow
from firmcrm.schemas import FirmCrmActivityCreate, FirmCrmActivityOut, FirmCrmActivityUpdate, FirmCrmPage
from firmcrm.services import visibility

router = APIRouter(route_class=FirmCrmRoute, prefix="/activities", tags=["activities"])


def _enrich(db: Session, rows: list[Activity]) -> list[FirmCrmActivityOut]:
    ...
    out = []
    for a in rows:
        d = FirmCrmActivityOut.model_validate(a)
...
@router.get("", response_model=FirmCrmPage[FirmCrmActivityOut])
def list_activities(kind: ActivityKind | None = None, account_id: int | None = None, contact_id: int | None = None,
                    opportunity_id: int | None = None, lead_id: int | None = None, owner_id: str | None = None,
...
@router.post("", response_model=FirmCrmActivityOut, status_code=201)
def create_activity(body: FirmCrmActivityCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
@router.patch("/{activity_id}", response_model=FirmCrmActivityOut)
def update_activity(activity_id: int, body: FirmCrmActivityUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
