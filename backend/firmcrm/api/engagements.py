...
from firmcrm.enums import EngagementStatus
from firmcrm.models import Account, Engagement, Opportunity, User
from firmcrm.schemas import FirmCrmEngagementCreate, FirmCrmEngagementOut, FirmCrmEngagementUpdate, FirmCrmPage
from firmcrm.services import visibility

router = APIRouter(route_class=FirmCrmRoute, prefix="/engagements", tags=["engagements"])


def _enrich(db: Session, rows: list[Engagement]) -> list[FirmCrmEngagementOut]:
    ...
    out = []
    for e in rows:
        d = FirmCrmEngagementOut.model_validate(e)
...
@router.get("", response_model=FirmCrmPage[FirmCrmEngagementOut])
...
@router.post("", response_model=FirmCrmEngagementOut, status_code=201)
def create_engagement(body: FirmCrmEngagementCreate, db: Session = Depends(get_db), actor: User = Depends(at_least("manager"))):
...
@router.patch("/{eng_id}", response_model=FirmCrmEngagementOut)
def update_engagement(eng_id: int, body: FirmCrmEngagementUpdate, db: Session = Depends(get_db), actor: User = Depends(at_least("manager"))):
