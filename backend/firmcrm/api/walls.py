...
from firmcrm.api.common import get_or_404, paginate, user_names
...
from firmcrm.models import Account, EthicalWall, EthicalWallMember, Opportunity, User, utcnow
from firmcrm.schemas import FirmCrmPage
from firmcrm.services import visibility

router = APIRouter(route_class=FirmCrmRoute, prefix="/walls", tags=["ethical-walls"])
EntityType = Literal["account", "opportunity"]


class FirmCrmWallMemberOut(BaseModel):
    user_id: str
    full_name: str
    role: str
    added_at: str
...
class FirmCrmWallOut(BaseModel):
    id: int
    ...
    members: list[FirmCrmWallMemberOut]


class FirmCrmWallCreate(BaseModel):
    entity_type: EntityType
    entity_id: int
...
class FirmCrmMemberIn(BaseModel):
    user_id: str
...
def _out(db: Session, walls: list[EthicalWall]) -> list[FirmCrmWallOut]:
    un = user_names(db, [w.created_by_id for w in walls])
    out = []
    for w in walls:
        out.append(FirmCrmWallOut(id=w.id, entity_type=w.entity_type, entity_id=w.entity_id, entity_name=_entity_name(db, w), reason=w.reason,
                           ...
                           members=[FirmCrmWallMemberOut(user_id=m.user_id, full_name=m.user.full_name, role=m.user.role, added_at=m.added_at.isoformat())
...
@router.get("", response_model=FirmCrmPage[FirmCrmWallOut])
...
@router.get("/for/{entity_type}/{entity_id}", response_model=FirmCrmWallOut | None)
...
@router.post("", response_model=FirmCrmWallOut, status_code=201)
def create_wall(body: FirmCrmWallCreate, db: Session = Depends(get_db), actor: User = Depends(require_role("partner", "admin"))):
...
@router.get("/{wall_id}", response_model=FirmCrmWallOut)
...
@router.post("/{wall_id}/members", response_model=FirmCrmWallOut)
def add_member(wall_id: int, body: FirmCrmMemberIn, db: Session = Depends(get_db), actor: User = Depends(require_role("partner", "admin"))):
...
@router.delete("/{wall_id}/members/{user_id}", response_model=FirmCrmWallOut)
def remove_member(wall_id: int, user_id: str, db: Session = Depends(get_db), actor: User = Depends(require_role("partner", "admin"))):
    w = _load(db, wall_id, actor)
...
@router.post("/{wall_id}/lift", response_model=FirmCrmWallOut)
