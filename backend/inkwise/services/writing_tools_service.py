...
def build_grounded_writing_tool_prompt(
    *,
    ...
    parts.append("- When possible, pin each citation to its support: after the ID, add the shortest verbatim quote from that evidence block that directly supports the sentence, like [E01|\"exact words copied from the evidence\"].")
    parts.append("- The pinned quote must be copied character-for-character from the evidence block text, at most about 40 words, with no double-quote characters inside it. If you cannot quote exactly, use the bare ID like [E01].")
...
def build_grounded_prediction_prompt(
    *,
    ...
    parts.append("- When possible, pin each citation to its support: after the ID, add the shortest verbatim quote from that evidence block that directly supports the clause, like [E01|\"exact words copied from the evidence\"].")
    parts.append("- The pinned quote must be copied character-for-character from the evidence block text, at most about 40 words, with no double-quote characters inside it. If you cannot quote exactly, use the bare ID like [E01].")
    parts.append("- Only cite IDs that appear in the grounded evidence below.")
    parts.append("- Evidence headers include modality and segment_type metadata.")
    parts.append("- If multiple evidence blocks overlap or say the same thing, cite the single most specific block.")
