...
class NormalizePredictionResultTests(unittest.TestCase):
    def test_returns_full_first_line_without_truncation(self) -> None:
        body = InkwisePredictionRequest(current_block_prefix_text="Prior context that should stay intact.")
    ...
    def test_rejects_duplicate_current_block_prefix(self) -> None:
        body = InkwisePredictionRequest(current_block_prefix_text="The quick brown fox")

        result = normalize_prediction_result(raw_text="The quick brown fox", body=body)

        self.assertEqual(result.text, "")
        self.assertEqual(result.reason, "duplicate_current_block_prefix")


class PredictionPromptTests(unittest.TestCase):
    def test_uses_cursor_aware_labels(self) -> None:
        body = InkwisePredictionRequest(
            current_block_prefix_text="The lease renews unless",
        )
        ...
        self.assertIn("Current block text before cursor:", prompt)
        self.assertIn("Current block text before cursor:", grounded_prompt)
        self.assertNotIn("Text before cursor:", prompt)
