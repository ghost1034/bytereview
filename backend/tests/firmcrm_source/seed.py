"""Source demo fixtures, used only in isolated regression tests."""
import random
...
from firmcrm.services import conflicts as conflict_svc

from models.db_models import User as PlatformUser
from firmcrm.models import FirmCrmSettings
R = random.Random(7)
NOW = utcnow()
TODAY = NOW.date()
def d(days_ago):
...
def seed_demo(db):
    db.info["settings"] = FirmCrmSettings(firm_id=db.info["firm_id"], access_revision=0)
    db.add(db.info["settings"])
    db.flush()
    # ---- practice areas
    pa_defs = [
    ...
    for email, name, role, title, pa in udefs:
        db.add(PlatformUser(id=email, firm_id=db.info["firm_id"], email=email, display_name=name, role="admin" if role == "admin" else "analyst"))
        db.flush()
        u = User(email=email, full_name=name, id=email, role=role, title=title,
