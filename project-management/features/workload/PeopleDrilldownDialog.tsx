...
export function PeopleDrilldownDialog({ open, onOpenChange, user, tasks }: { open: boolean; onOpenChange: (open: boolean) => void; user?: User; tasks: Task[] }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent aria-describedby={undefined} data-workload-people-drilldown>
    ...
    <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>{user.jobTitle ?? user.email}</p>
    ...
    <section><h3 className="mb-2 text-sm font-semibold">Today&apos;s work</h3>{todayTasks.length ? <ul className="space-y-1 text-sm">{todayTasks.map((task) => <li key={task.id}>{task.name}</li>)}</ul> : <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>No scheduled work today.</p>}</section>
    <section><h3 className="mb-2 text-sm font-semibold">Capacity history</h3><p className="text-sm" style={{ color: 'var(--ink-muted)' }}>Current capacity {userWeeklyCapacity(user)}h/week · {user.timeOff?.length ?? 0} recorded time-off period(s).</p></section>
