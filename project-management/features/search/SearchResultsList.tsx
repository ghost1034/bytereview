...
export function SearchResultsList({
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'goals' ? (
        <div className="overflow-hidden rounded-xl border" style={{ borderColor: 'var(--border-subtle)' }}>
          <div className="grid grid-cols-[minmax(220px,1fr)_140px_120px_120px] gap-2 border-b px-3 py-2 text-[10px] font-semibold uppercase tracking-wide" style={{ background: 'var(--bg-muted)', color: 'var(--ink-muted)' }}>
            <span>Goal</span><span>Owner</span><span>Progress</span><span>Status</span>
          </div>
          {goalRows.map((goal) => {
            const progress = computeGoalProgress(goal.id, goalRows, [])
            return <button key={goal.id} type="button" onClick={() => router.push(`${basePath}/goals/${goal.id}`)} className="grid w-full grid-cols-[minmax(220px,1fr)_140px_120px_120px] gap-2 border-b px-3 py-2 text-left text-sm" style={{ borderColor: 'var(--border-subtle)' }}>
          ...
          {!goalRows.length ? <p className="px-4 py-8 text-center text-sm" style={{ color: 'var(--ink-muted)' }}>No goals match your search.</p> : null}
        </div>
      ) : null}

      {tab === 'people' ? (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {peopleRows.map((person) => <div key={person.id} className="rounded-xl border p-4" style={{ borderColor: 'var(--border-subtle)' }}>
            <p className="font-medium"><HighlightText text={person.name} query={query} /></p>
            <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>{person.email}</p>
          ...
          {!peopleRows.length ? <p className="text-sm" style={{ color: 'var(--ink-muted)' }}>No people match your search.</p> : null}
        </div>
      ) : null}
    </div>
  )
