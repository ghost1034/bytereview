...
export function SearchResultsBoard({ tasks, projects }: { tasks: Task[]; projects: Project[] }) {
  ...
  return <div className="flex gap-3 overflow-x-auto" data-search-view="board">
    {columns.map(({ project, tasks: rows }) => <section key={project.id} className="min-w-64 rounded-xl border p-3" style={{ borderColor: 'var(--border-subtle)' }}>
      <h2 className="text-sm font-semibold">{project.name} <span className="font-normal">({rows.length})</span></h2>
      <div className="mt-2 space-y-2">{rows.map((task) => <div key={task.id} className="rounded-lg border p-2 text-sm" style={{ borderColor: 'var(--border-subtle)' }}>{task.name}</div>)}</div>
    </section>)}
  </div>
}
