...
export function ApprovalsSettingsPage() {
  ...
  return <div className="space-y-5">
    <div><h1 className="font-serif text-2xl">Approvals</h1><p className="text-sm" style={{ color: 'var(--ink-muted)' }}>One policy surface for time, timesheets, expenses, and reimbursements.</p></div>
    <div className="tl-card max-w-2xl space-y-4 p-5 shadow-paper-sm">
      <label className="flex items-center justify-between gap-4 text-sm"><span><b>Time approval</b><br /><span style={{ color: 'var(--ink-muted)' }}>Require submitted time and timesheets to be approved.</span></span><Switch checked={timeRequired} disabled={!editable} onCheckedChange={setTimeRequired} /></label>
      <label className="flex items-center justify-between gap-4 text-sm"><span><b>Expense approval</b><br /><span style={{ color: 'var(--ink-muted)' }}>Require reports before reimbursement or client billing.</span></span><Switch checked={expenseRequired} disabled={!editable} onCheckedChange={setExpenseRequired} /></label>
      ...
      <p className="text-xs" style={{ color: 'var(--ink-muted)' }}>Approval buttons are additionally restricted by each member’s Approve capability. Lock, write-off, and reimbursement require Billing capability.</p>
