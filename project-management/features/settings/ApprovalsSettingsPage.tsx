...
export function ApprovalsSettingsPage() {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-lg border border-border bg-card text-card-foreground max-w-2xl space-y-4 p-5 shadow-sm">
      ...
      <label className="grid gap-1 text-sm"><span>Receipt required above</span><Input className="rounded-md border border-input bg-background text-foreground max-w-40 font-mono" type="number" min="0" disabled={!editable} value={receiptThreshold} onChange={(e) => setReceiptThreshold(e.target.value)} /></label>
      ...
      {editable && <Button className=" border-0" onClick={() => void save()}>Save approval settings</Button>}
    </div>
  </div>
}
