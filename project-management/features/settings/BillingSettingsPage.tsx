...
function ReadOnlyNotice() { return <p className="tl-card p-4 text-sm">You can view billing configuration, but changing it requires rate-management permission.</p> }
...
function ActivityCodes({ workspaceId, editable }: { workspaceId: string; editable: boolean }) {
  ...
  return <section className="space-y-3">{editable && <div className="tl-card flex flex-wrap gap-3 p-4"><Input aria-label="Activity code" className="max-w-40" placeholder="Code" value={code} onChange={(event) => setCode(event.target.value)} /><Input aria-label="Activity name" className="max-w-xs" placeholder="Description" value={name} onChange={(event) => setName(event.target.value)} /><Button onClick={() => void create()}>Add code</Button><Button variant="outline" onClick={() => void loadUtbms()}>Load UTBMS defaults</Button></div>}<div className="tl-card divide-y">{codes.map((item) => <div className="flex justify-between p-3 text-sm" key={item.id}><span><b className="font-mono">{item.code}</b> · {item.name}</span><span>{item.active ? 'Active' : 'Inactive'}</span></div>)}</div></section>
}
...
function Budgets({ workspaceId, editable }: { workspaceId: string; editable: boolean }) {
  ...
  return <section className="space-y-3">{editable && <div className="tl-card grid gap-3 p-4 md:grid-cols-4"><Input aria-label="Budget scope ID" placeholder="Matter / project ID" value={scopeId} onChange={(event) => setScopeId(event.target.value)} /><Input aria-label="Budget amount" placeholder="Amount" value={amount} onChange={(event) => setAmount(event.target.value)} /><Input aria-label="Budget hours" placeholder="Hours" value={hours} onChange={(event) => setHours(event.target.value)} /><Button onClick={() => void create()}>Add budget</Button></div>}<div className="tl-card divide-y">{budgets.map((budget) => <div className="flex justify-between p-3 text-sm" key={budget.id}><span>{budget.scope}: {budget.scopeId}</span><span className="font-mono">{budget.amount ? formatMoney(budget.amount, budget.currency) : '—'} · {budget.hours ?? '—'}h</span></div>)}</div></section>
}
...
function FxOverrides({ editable }: { editable: boolean }) {
  ...
  return <section className="space-y-3"><p className="text-sm">ECB daily rates are cached automatically. Add a workspace quote only for unsupported currency pairs.</p>{editable && <div className="tl-card flex flex-wrap gap-3 p-4"><Input aria-label="FX pair" className="max-w-36" value={pair} onChange={(event) => setPair(event.target.value)} /><Input aria-label="FX rate" className="max-w-36" placeholder="Rate" value={rate} onChange={(event) => setRate(event.target.value)} /><Input aria-label="FX note" className="max-w-xs" placeholder="Source / note" value={note} onChange={(event) => setNote(event.target.value)} /><Button onClick={() => void add()}>Add FX override</Button></div>}<div className="tl-card divide-y">{Object.entries(workspace.fxOverrides ?? {}).map(([key, value]) => <div className="flex justify-between p-3 text-sm" key={key}><span className="font-mono">{key}</span><span>{value.rate} · {value.effectiveOn} {value.note && `· ${value.note}`}</span></div>)}</div></section>
}
