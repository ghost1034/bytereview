...
export function BundlesPanel({ workspaceId, userId }: { workspaceId: string; userId: string }) {
  ...
  return <div className="space-y-4">
    <div className="grid gap-2 rounded-xl border p-4 sm:grid-cols-[80px_1fr_1fr_auto]" style={{ borderColor: 'hsl(var(--border))' }}>
    ...
    <ul className="space-y-2">{bundles.map((bundle) => <li key={bundle.id} className="flex items-center justify-between rounded-xl border p-4" style={{ borderColor: 'hsl(var(--border))' }}><div><p className="font-medium">{bundle.iconEmoji} {bundle.name}</p><p className="text-xs" style={{ color: 'hsl(var(--foreground-muted))' }}>{bundle.sectionNames.length} sections · applied to {bundle.appliedToProjectIds.length} projects</p></div><div className="flex gap-1"><Button variant="ghost" size="sm" onClick={() => { setEditingId(bundle.id); setName(bundle.name); setIcon(bundle.iconEmoji ?? '🧰'); setSections(bundle.sectionNames.join(', ')) }}>Edit</Button><Button variant="ghost" size="sm" onClick={() => void remove(bundle.id)}>Delete</Button></div></li>)}</ul>
  </div>
}
