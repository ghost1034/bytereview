...
function ViewPreview({ kind }: { kind: ProjectView }) {
  ...
  return (
    <svg viewBox="0 0 48 32" className="h-8 w-12" aria-hidden>
      <rect x="2" y="2" width="44" height="28" rx="4" fill="var(--bg-muted)" />
      {kind === 'board' &&
        [0, 1, 2].map((i) => (
          <rect key={i} x={6 + i * 14} y="8" width="10" height="16" rx="2" fill="var(--primary-soft)" />
        ))}
      {kind === 'calendar' && (
        <>
          <rect x="8" y="8" width="32" height="6" rx="2" fill="var(--border-subtle)" />
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <rect key={i} x={8 + (i % 3) * 11} y={16 + Math.floor(i / 3) * 7} width="8" height="5" rx="1" fill="var(--accent-soft)" />
          ))}
        </>
      )}
      {kind === 'timeline' && (
        <>
          <line x1="8" y1="22" x2="40" y2="22" stroke="var(--border-default)" />
          {[10, 22, 34].map((x) => (
            <circle key={x} cx={x} cy="22" r="3" fill="var(--primary)" />
          ))}
        </>
      )}
      {kind === 'gantt' && (
        <>
          <rect x="8" y="10" width="20" height="4" rx="2" fill="var(--primary-soft)" />
          <rect x="14" y="18" width="24" height="4" rx="2" fill="var(--accent-soft)" />
        </>
      )}
      {kind === 'list' &&
        Array.from({ length: bars }).map((_, i) => (
          <rect key={i} x="8" y={8 + i * 5} width={28 - i * 4} height="3" rx="1.5" fill="var(--primary-soft)" />
        ))}
    </svg>
  )
}
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {PROJECT_VIEWS.map((view) => {
        ...
        return (
          <div
            key={view}
            className="tl-card p-3 shadow-paper-sm"
          ...
          >
            ...
            >
              ...
              <div>
                ...
                {isDefault && (
                  <p className="text-xs" style={{ color: 'var(--primary)' }}>Default view</p>
                )}
              </div>
            </button>
            <label className="mt-2 flex items-center gap-2 text-xs" style={{ color: 'var(--ink-muted)' }}>
              <input
                type="checkbox"
                checked={enabled}
