...
export function MatterDetailPage({ matterId }: { matterId: string }) {
  ...
  return <div className="space-y-5">
    <div><p className="font-mono text-xs" style={{ color: 'hsl(var(--foreground-muted))' }}>{matter.matterNumber}</p><h1 className="font-sans text-2xl">{project?.name ?? terms.singular}</h1><Link className="text-sm underline" href={`/dashboard/project-management/w/${workspaceId}/psa/clients/${matter.clientId}`}>{client?.name ?? 'Client'}</Link></div>
