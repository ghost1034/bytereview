...
export function MatterDetailPage({ matterId }: { matterId: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="tl-card grid gap-3 p-4 text-sm shadow-sm sm:grid-cols-2"><p><b>Practice area</b><br />{matter.practiceArea}</p><p><b>Fee arrangement</b><br />{matter.feeArrangement}</p><p><b>Opened</b><br />{matter.openedAt}</p><p><b>Conflict status</b><br />{matter.conflictStatus}</p></div>
  </div>
}

function Metric({ label, value }: { label: string; value: string }) { return <div className="tl-card p-4 shadow-sm"><p className="text-xs uppercase" style={{ color: 'hsl(var(--foreground-muted))' }}>{label}</p><p className="font-mono text-lg">{value}</p></div> }
