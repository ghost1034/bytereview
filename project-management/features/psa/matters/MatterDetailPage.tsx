...
export function MatterDetailPage({ matterId }: { matterId: string }) {
  ...
  return <div className="space-y-5">
    <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs" style={{ color: 'hsl(var(--foreground-muted))' }}>{matter.matterNumber}</p><h1 className="font-sans text-2xl">{project?.name ?? terms.singular}</h1><Link className="text-sm underline" href={`/dashboard/project-management/w/${workspaceId}/psa/clients/${matter.clientId}`}>{client?.name ?? 'Client'}</Link></div><Button variant="outline" size="sm" onClick={() => setEditOpen(true)}><Pencil className="mr-1 h-3.5 w-3.5" /> Edit {terms.singular.toLowerCase()}</Button></div>
    ...
    <div className="rounded-lg border border-border bg-card text-card-foreground grid gap-3 p-4 text-sm shadow-sm sm:grid-cols-2"><p><b>Practice area</b><br />{matter.practiceArea}</p><p><b>Fee arrangement</b><br />{matter.feeArrangement}</p><p><b>Opened</b><br />{matter.openedAt}</p><p><b>Conflict status</b><br />{matter.conflictStatus}</p></div>
    {editOpen && <MatterDialog open onOpenChange={setEditOpen} workspaceId={workspaceId} matter={matter} />}
  </div>
}

function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-lg border border-border bg-card text-card-foreground p-4 shadow-sm"><p className="text-xs uppercase" style={{ color: 'hsl(var(--foreground-muted))' }}>{label}</p><p className="font-mono text-lg">{value}</p></div> }
