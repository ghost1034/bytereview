      ...
      <DialogContent aria-describedby={undefined} className="max-h-[92vh] max-w-5xl overflow-y-auto">
        ...
        {step === 4 && preview && (
          <div className="max-h-64 space-y-2 overflow-y-auto py-2 text-sm">
            ...
            <div>
              <Label htmlFor="invoice-tax-amount">Tax</Label>
              <Input id="invoice-tax-amount" value={taxAmount} onChange={(e) => setTaxAmount(e.target.value)} className="rounded-md border border-input bg-background text-foreground font-mono tabular-nums" />
            </div>
            <div>
              <Label htmlFor="invoice-discount">Discount</Label>
              <Input id="invoice-discount" value={discount} onChange={(e) => setDiscount(e.target.value)} className="rounded-md border border-input bg-background text-foreground font-mono tabular-nums" />
            </div>
            <div>
              <Label htmlFor="invoice-discount-reason">Discount reason</Label>
              <Input id="invoice-discount-reason" value={discountReason} onChange={(e) => setDiscountReason(e.target.value)} className="rounded-md border border-input bg-background text-foreground" />
            </div>
          </div>
        )}
