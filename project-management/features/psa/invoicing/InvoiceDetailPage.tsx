...
export function InvoiceDetailPage({ invoiceId }: { invoiceId: string }) {
  ...
  return <div className="space-y-4" data-tour-page="invoice-detail">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><Link className="text-sm underline" href={`/dashboard/project-management/w/${workspaceId}/psa/invoicing`}>Back to invoices</Link><div className="mt-2 flex items-center gap-2"><h1 className="font-serif text-2xl">{invoice.invoiceNumber}</h1><Badge variant="outline">{invoice.status.replace(/_/g, ' ')}</Badge></div><p className="text-sm">{invoice.clientName} · issued {invoice.issueDate} · due {invoice.dueOn}</p></div><div className="flex flex-wrap gap-2">
    ...
    {paymentLink ? <div className="tl-card flex flex-wrap items-center gap-2 p-3" role="status"><span className="text-sm">Client invoice payment link ready.</span><Input aria-label="Client invoice payment link" className="min-w-64 flex-1" readOnly value={paymentLink} /><Button variant="outline" onClick={() => void navigator.clipboard.writeText(paymentLink)}>Copy</Button></div> : null}
    <div className="grid gap-4 lg:grid-cols-[1fr_20rem]"><main className="space-y-4"><div className="tl-card p-4"><h2 className="font-medium">Invoice narrative</h2>{invoice.status === 'draft' && canBill ? <><Textarea aria-label="Invoice narrative" className="mt-2" value={narrative} onChange={(event) => setNarrative(event.target.value)} /><Textarea aria-label="Invoice notes" className="mt-2" value={notes} onChange={(event) => setNotes(event.target.value)} /><div className="mt-2 grid gap-2 sm:grid-cols-2"><Input aria-label="Discount amount" value={discount} onChange={(event) => setDiscount(event.target.value)} /><Input aria-label="Discount reason" value={discountReason} onChange={(event) => setDiscountReason(event.target.value)} /></div></> : <><p className="mt-2 text-sm">{invoice.narrative || 'No summary narrative.'}</p><p className="mt-2 text-sm" style={{ color: 'var(--ink-muted)' }}>{invoice.notes}</p></>}</div><div className="tl-card overflow-hidden"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="px-4 py-2">Description</th><th className="px-4 py-2 text-right">Qty</th><th className="px-4 py-2 text-right">Rate</th><th className="px-4 py-2 text-right">Amount</th></tr></thead><tbody>{invoice.lineItems.map((line, index) => <tr className="border-b" key={line.id ?? `${line.sourceId}-${index}`}><td className="px-4 py-2">{line.description}</td><td className="px-4 py-2 text-right font-mono">{line.quantity}</td><td className="px-4 py-2 text-right font-mono">{formatMoney(line.rate, invoice.currency)}</td><td className="px-4 py-2 text-right font-mono">{formatMoney(line.amount ?? line.quantity * line.rate, invoice.currency)}</td></tr>)}</tbody><tfoot><tr><td className="px-4 py-2" colSpan={3}>Discount</td><td className="px-4 py-2 text-right font-mono">−{formatMoney(invoice.discountAmount ?? 0, invoice.currency)}</td></tr><tr className="font-medium"><td className="px-4 py-2" colSpan={3}>Total</td><td className="px-4 py-2 text-right font-mono">{formatMoney(invoice.total ?? invoice.amount, invoice.currency)}</td></tr></tfoot></table></div><div className="tl-card p-4"><h2 className="font-medium">Audit history</h2><div className="mt-2 space-y-2">{audit.sort((a, b) => b.at.localeCompare(a.at)).map((entry) => <div className="flex justify-between text-sm" key={entry.id}><span>{entry.action.replace(/_/g, ' ')}</span><span className="font-mono text-xs">{entry.at.slice(0, 19).replace('T', ' ')}</span></div>)}{audit.length === 0 && <p className="text-sm">No billing events.</p>}</div></div></main><aside className="space-y-4"><div className="tl-card p-4"><h2 className="font-medium">Balance</h2><p className="mt-2 font-mono text-2xl">{formatMoney(invoice.amountOutstanding ?? invoice.amount, invoice.currency)}</p><p className="text-sm">{age} days old · {new Date(invoice.dueOn) < new Date() && (invoice.amountOutstanding ?? 0) > 0 ? 'past due' : 'current'}</p></div><div className="tl-card p-4"><h2 className="font-medium">Payments</h2><div className="mt-2 space-y-3">{activePayments.map((payment) => <div className="text-sm" key={payment.id}><div className="flex justify-between"><span>{payment.method.replace(/_/g, ' ')}</span><span className="font-mono">{formatMoney(payment.amount, payment.currency)}</span></div><p className="text-xs">{payment.paidAt} {reversedIds.has(payment.id) ? '· reversed' : ''}</p>{canPay && !reversedIds.has(payment.id) && <Button className="mt-1 h-7" size="sm" variant="ghost" onClick={() => { const reason = window.prompt('Payment reversal reason'); if (reason) void reverseInvoicePayment(payment.id, workspaceId, reason) }}>Reverse</Button>}</div>)}</div></div></aside></div>
