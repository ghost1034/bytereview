...
- FirmCRM Settings opens the shared Firm management panel: the same firm ID, invitation code, directory, and platform Admin/User controls used by AI Analytics Suite. Members joining with that code also appear in FirmCRM, Tasklytic, and PBC. Only platform firm administrators can generate codes or change platform roles; a CRM-only administrator does not gain those permissions. Administration links to these settings and retains CRM-specific roles and access. The CRM settings tab contains business rules.
