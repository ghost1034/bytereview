...
import { dataApi, engagementsApi } from "@/api";
import { ArrowUpRight, Download } from "lucide-react";
import { Pagination, usePager } from "@/components/ui/Pagination";
import type { Engagement } from "@/api/types";
import { Badge, Button, Empty, PageHeader, Select, statusTone } from "@/components/ui";
...
import { useUsers, opt, strOpts } from "@/lib/hooks";
import { ENGAGEMENT_STATUSES, FEE_TYPES } from "@/lib/options";
import { money, titleCase } from "@/lib/format";
import { useAuth } from "@/lib/auth";
import { NameCell, ResultCount, cellDate, cellMoney, cellText } from "@/components/ui/cells";
...
export default function EngagementsPage() {
  ...
  const cols: Column<Engagement>[] = [
    ...
    { key: "name", header: "Engagement · client", sort: (e) => e.name, render: (e) => <NameCell name={e.name} max={360} sub={<span className="inline-flex items-center gap-1.5">
        ...
        <Link to={`/accounts/${e.account_id}`} onClick={(ev) => ev.stopPropagation()} className="truncate text-sand-700 hover:text-sand-900 hover:underline" title={e.account_name ?? undefined}>{e.account_name}</Link>
        {e.opportunity_id && <Link to={`/opportunities/${e.opportunity_id}`} onClick={(ev) => ev.stopPropagation()} className="inline-flex shrink-0 items-center gap-0.5 text-sand-700 hover:text-sand-900 hover:underline">Source opp<ArrowUpRight size={11} aria-hidden /></Link>}
      </span>} /> },
