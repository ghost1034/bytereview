...
import { oppsApi, reportsApi } from "@/api";
import { Badge, Card, Empty, PageHeader, Stat, cn } from "@/components/ui";
import { CHART, ChartFrame, ChartLegend, WinLossBar, barChartProps, barProps, gridProps, moneyTooltipFormatter, tooltipProps, xAxisProps, yAxisMoneyProps } from "@/components/crm/charts";
import { fmtDate, money, num, pct } from "@/lib/format";
...
function Dash() { return <span className="text-sand-300">—</span>; }

/* Compact money for tiles: one decimal ($3.1M), unlike the 0-decimal shared helper. */
const compact = (n: number | null | undefined) => (n == null ? "—" : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", notation: "compact", maximumFractionDigits: 1 }).format(n));
...
function Due({ iso }: { iso: string | null }) {
  if (!iso) return <span className="text-[12px] leading-4 text-sand-400">No date</span>;
  const d = parseISO(iso);
  const today = isToday(d);
  const over = !today && isBefore(d, startOfDay(new Date()));
  return (
    <span className={cn("whitespace-nowrap text-[12px] leading-4 num", over ? "font-medium text-danger-600" : today ? "font-medium text-warn-700" : "text-sand-500")}>
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={title}
        subtitle={<>Firm-wide business development snapshot<span className="mx-1.5 text-sand-300">·</span>Data as of <span className="num">{asOf}</span></>}
      />
      ...
      <div data-tour="kpis" className="grid grid-cols-2 items-stretch gap-4 min-[960px]:grid-cols-3 min-[1280px]:grid-cols-6">
        <Stat label="Open pipeline" value={compact(k.open_pipeline)} sub={plural(k.open_count, "opportunity", "opportunities")} />
        <Stat label="Weighted pipeline" value={compact(k.weighted_pipeline)} sub="probability-adjusted" />
        <Stat label="Won QTD" value={compact(k.won_qtd)} delta={k.won_qtd_delta_pct} sub={`${plural(k.won_qtd_count, "win", "wins")} · ${compact(k.won_mtd)} MTD${k.won_qtd_delta_pct != null ? " · vs. prior quarter to date" : ""}`} />
        <Stat label="Closing ≤ 30 days" value={compact(k.closing_30_amount)} sub={plural(k.closing_30_count, "opportunity", "opportunities")} />
        <Stat label="Stale opportunities" value={num(k.stale_count)} sub="no activity ≥ 21 days" tone={k.stale_count ? "warn" : "default"} />
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        >
          ...
          <div className="-mx-5 -mb-5 mt-4 overflow-hidden rounded-b-lg border-t border-sand-150">
        ...
        <div className="col-span-4 flex flex-col gap-4">
          <Card title="My open tasks" actions={<Link to="/tasks" className="text-[12px] font-medium">All →</Link>}>
            {d.data.my_tasks.length === 0 ? (
              <div className="text-[12px] leading-4 text-sand-500">No open tasks.</div>
            ) : (
              <ul className="-mt-2">
                {d.data.my_tasks.map((t) => {
                  ...
                  return (
                    <li key={t.id} className="grid grid-cols-[16px_1fr_auto] items-start gap-2.5 border-b border-sand-100 py-2 last:border-b-0">
                      <span className="mt-0.5 h-4 w-4 rounded-[3px] border border-sand-300 bg-sand-0" aria-hidden />
                      <div className="min-w-0">
                        <button type="button" className="block w-full truncate text-left text-[13px] font-medium text-sand-900 hover:underline" onClick={() => nav(to)}>{t.subject}</button>
                        {(high || meta) && (
                          <div className="flex min-w-0 items-center gap-1.5 text-[12px] leading-4 text-sand-500">
          ...
          <Card title="Win / loss" actions={<span className="text-sand-500">Trailing 6 months</span>}>
            <div className="grid grid-cols-3 gap-3">
              <div><div className="text-[18px] leading-6 font-semibold tracking-[-0.015em] text-success-600 num">{pct(wl.win_rate)}</div><div className="text-[12px] leading-4 text-sand-500">Win rate</div></div>
              <div><div className="text-[18px] leading-6 font-semibold tracking-[-0.015em] num">{num(wl.won_count)}</div><div className="text-[12px] leading-4 text-sand-500">Won</div></div>
              <div><div className="text-[18px] leading-6 font-semibold tracking-[-0.015em] num">{num(wl.lost_count)}</div><div className="text-[12px] leading-4 text-sand-500">Lost</div></div>
            </div>
            <WinLossBar won={wl.won_count} lost={wl.lost_count} className="mt-3.5" />
            <div className="mt-2.5 flex justify-between text-[12px] leading-4 text-sand-600">
              <span>Avg. won <b className="font-medium text-sand-900 num">{money(wl.avg_won_amount)}</b></span>
              <span>Avg. <b className="font-medium text-sand-900 num">{wl.avg_days_to_close == null ? "—" : `${Math.round(wl.avg_days_to_close)} days`}</b> to close</span>
            </div>
          </Card>
        </div>
      </div>
      ...
      <Card
        ...
        padded={false}
        actions={<span className="text-sand-500">No activity in 21 or more days<span className="mx-1.5 text-sand-300">·</span><span className="num">{num(k.stale_count)} of {num(k.open_count)}</span></span>}
      >
        {stale.isLoading ? <TableSkeleton /> : !stale.data?.length ? <Empty title="No stale opportunities" hint="Every open pursuit has had activity in the last 21 days." /> : (
          <>
              ...
              <tbody>
                {stale.data.map((o) => (
                  <tr key={o.id} className="clickable" onClick={() => nav(`/opportunities/${o.id}`)}>
                    ...
                    <td className={cn("text-right num", o.days_in_stage >= 21 && "font-medium text-warn-700")}>{num(o.days_in_stage)}</td>
                    <td className="whitespace-nowrap text-right num text-sand-600">{fmtDate(o.last_activity_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="flex h-11 items-center justify-between border-t border-sand-150 px-5 text-[12px] leading-4 text-sand-500">
