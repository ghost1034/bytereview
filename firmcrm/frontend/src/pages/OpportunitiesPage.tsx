...
import { accountsApi, contactsApi, dataApi, oppsApi } from "@/api";
import { Pagination, usePager } from "@/components/ui/Pagination";
import { useAuth } from "@/lib/auth";
import type { Opportunity, Stage } from "@/api/types";
import { Badge, Button, Empty, Input, PageHeader, Select, cn, statusTone } from "@/components/ui";
import { DataTable, useServerSort, type Column } from "@/components/ui/DataTable";
import { ArchivedChip, FilterToggle, NameCell, cellMoney, cellText } from "@/components/ui/cells";
import { useMediaQuery } from "@/components/ui/useMediaQuery";
import { FormModal, type FieldDef } from "@/components/ui/Form";
import { useToast } from "@/components/ui/Toast";
import { ClearanceShield, KanbanCard, StaleChip, isPastDue } from "@/components/crm/KanbanCard";
import { paLabel, usePipelines, usePracticeAreas, useUsers, opt, strOpts } from "@/lib/hooks";
import { EL_STATUSES, FEE_TYPES } from "@/lib/options";
...
export function useOpportunityFields(accountId?: number): FieldDef[] {
  ...
  return useMemo(() => [
    ...
    { name: "amount", label: "Estimated fees ($)", type: "number", required: true }, { name: "fee_type", label: "Fee type", type: "select", options: strOpts(FEE_TYPES) },
...
function ViewToggle({ view, onChange }: { view: "board" | "table"; onChange: (v: "board" | "table") => void }) {
  const btn = (v: "board" | "table", Icon: typeof LayoutGrid, label: string) => (
    <button type="button" onClick={() => onChange(v)} aria-pressed={view === v} aria-label={label} title={label}
            className={cn("grid h-8 w-8 place-items-center border border-sand-200 transition-colors duration-[120ms] first:rounded-l-md last:-ml-px last:rounded-r-md", view === v ? "bg-sand-100 text-sand-900" : "bg-sand-0 text-sand-600 hover:text-sand-900")}>
      <Icon size={14} />
    </button>
  );
  ...
  const zone = (s: Stage | undefined, kind: "won" | "lost") => {
    ...
    return (
      <div data-tour={kind === "won" ? "drop-won" : undefined} onDragOver={(e) => onDragOver(s, e)} onDragLeave={onDragLeave} onDrop={(e) => onDrop(s, e)}
           className={cn("flex h-14 items-center justify-center gap-2 rounded-lg border border-dashed text-[12px] leading-4 font-medium transition-colors duration-[120ms]",
             !over && "border-sand-300 text-sand-500",
             over && kind === "won" && "border-solid border-success-600 bg-success-50 text-success-700",
             over && kind === "lost" && "border-solid border-danger-600 bg-danger-50 text-danger-700")}>
        <Icon size={16} strokeWidth={1.75} />
        <span>Drop to mark {kind === "won" ? "Won" : "Lost"}</span>
        <span className={cn("font-normal", over ? (kind === "won" ? "text-success-600" : "text-danger-600") : "text-sand-500")}>
  ...
  const cols: Column<Opportunity>[] = ([
    ...
    { key: "stage", header: "Stage", width: "140px", nowrap: true, sort: (o) => o.stage_position ?? 0, render: (o) => (
      <div className="min-w-0">
        ...
        <div className="flex h-5 items-center text-[12px] leading-4 text-sand-500 num">{o.is_stale ? <StaleChip o={o} /> : o.status === "open" ? `${o.days_in_stage}d in stage` : null}</div>
      </div>) },
    ...
    { key: "close", header: "Expected close", width: "120px", align: "right", nowrap: true, sort: (o) => o.expected_close ?? "", render: (o) => <span className={cn("num", isPastDue(o) ? "font-medium text-danger-600" : "font-normal")}>{fmtDate(o.expected_close)}</span> },
  ...
  return (
    <div className="flex flex-col">
      <PageHeader title="Opportunities"
        subtitle={<><b className="font-medium text-sand-700 num">{opps.data?.total ?? 0}</b> {status === "all" ? "" : `${status} `}opportunities{view === "table" && opps.data && opps.data.total > pager.limit ? ` (showing ${rows?.length})` : ""}{dot}<b className="font-medium text-sand-700 num">{money(total)}</b> total{dot}<b className="font-medium text-sand-700 num">{money(weighted)}</b> weighted</>}
        actions={<>
            ...
            {visibleStages.map((s) => {
              ...
              return (
                <section key={s.id} className="flex min-w-[216px] flex-1 basis-0 flex-col" aria-label={`${s.name} stage`}>
                  <header className="mb-3 flex h-9 shrink-0 items-baseline gap-2 border-b border-sand-150 pr-1 pl-1.5">
                    {/* Icon is inline (vertical-align) so the h3 keeps a text baseline and the count/sum line up across columns. */}
                    <h3 className="whitespace-nowrap text-[13px] leading-5 font-semibold text-sand-900">{isClearanceStage(s) && <ShieldCheck size={14} className="mr-1.5 inline-block align-[-2px] text-sand-500" strokeWidth={1.75} aria-hidden />}{s.name}</h3>
                    <span className="mono text-sand-500">{items.length}</span>
                    <span className="ml-auto text-[12px] leading-4 font-medium text-sand-600 num">{money(sum)}</span>
                    <span className="mono text-[11px] text-sand-500">{s.probability}%</span>
                  </header>
                  <div onDragOver={(e) => onDragOver(s, e)} onDragLeave={() => setDragOver(null)} onDrop={(e) => onDrop(s, e)}
                       className={cn("flex min-h-[120px] flex-1 flex-col gap-2 rounded-lg p-0.5 transition-colors duration-[120ms]", over && "bg-accent-50 shadow-[inset_0_0_0_1px_var(--color-accent-300)]")}>
                    {items.map((o) => <KanbanCard key={o.id} o={o} onOpen={(x) => nav(`/opportunities/${x.id}`)} />)}
                    {items.length === 0 && <div className="grid h-[72px] place-items-center rounded-lg border border-dashed border-sand-200 text-[12px] text-sand-500">{over ? "Release to move here" : "No opportunities"}</div>}
                  </div>
                </section>);
            })}
          </div>
      ...
      <div className="mt-3 text-[11px] leading-[14px] text-sand-500">{titleCase(status)} view · drag cards between stages. Closed Won requires a cleared clearance check (where applicable) and a signed engagement letter.</div>
    </div>
  );
}
    ...
    <div className="flex gap-4" aria-busy="true" aria-label="Loading board">
      {Array.from({ length: 5 }).map((_, c) => (
        <div key={c} className="flex min-w-[216px] flex-1 basis-0 flex-col">
          <div className="mb-3 flex h-9 items-center border-b border-sand-150 px-1.5"><span className="skeleton w-2/5" /></div>
          <div className="flex flex-col gap-2 p-0.5">
            {Array.from({ length: 2 + (c % 2) }).map((_, i) => (
              <div key={i} className="space-y-2.5 rounded-lg border border-sand-150 bg-sand-0 p-3"><span className="skeleton w-4/5" /><span className="skeleton w-2/5" /><span className="skeleton w-3/5" /></div>
            ))}
          </div>
        </div>
      ))}
