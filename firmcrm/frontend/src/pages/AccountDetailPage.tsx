...
import { accountsApi, contactsApi, conflictsApi, engagementsApi, oppsApi } from "@/api";
import type { Account, Contact, Engagement, Opportunity } from "@/api/types";
import { Badge, Button, Card, DL, Empty, OverflowMenu, PageHeader, Spinner, Tabs, statusTone, type MenuItem } from "@/components/ui";
...
import { Dash, NameCell, cellDate, cellMoney, cellText } from "@/components/ui/cells";
...
export default function AccountDetailPage() {
  ...
  const riskTone = a.risk_rating === "high" ? "text-danger-700" : a.risk_rating === "medium" ? "text-warn-700" : "text-sand-900";
  const facts: Fact[] = [
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={<>{a.industry ?? "No industry"} · {titleCase(a.entity_kind)} · {a.city ? `${a.city}, ${a.state}` : "No location"} · Relationship partner <span className="text-sand-700">{a.owner_name ?? "unassigned"}</span></>}
        actions={<>
      ...
      <div className="mt-5">
        {tab === "overview" && (
          <div className="grid grid-cols-12 items-start gap-4">
            <Card title="Details" className="col-span-8"><DL columns={3} items={[
            ...
            <div className="col-span-4 space-y-4">
              <WallPanel entityType="account" id={id} entityName={a.name} />
