...
import { Badge, PageHeader, Tabs, cn } from "@/components/ui";
...
import { Dash, cellText } from "@/components/ui/cells";
...
function DoneControl({ done, onToggle }: { done: boolean; onToggle: () => void }) {
  return (
    <button type="button" role="checkbox" aria-checked={done} aria-label={done ? "Reopen task" : "Mark task done"}
            ...
            className="group -ml-1.5 grid h-7 w-7 place-items-center rounded-md transition-colors duration-[120ms] hover:bg-sand-100 focus-visible:outline-2 focus-visible:outline-accent-600">
      <span className={cn("grid h-4 w-4 place-items-center rounded-[3px] border transition-colors duration-[120ms]", done ? "border-accent-600 bg-accent-600 text-white group-hover:bg-accent-700" : "border-sand-300 bg-sand-0 group-hover:border-sand-400")} aria-hidden>
...
export default function TasksPage() {
  ...
  const cols: Column<Activity>[] = [
    ...
    { key: "subject", header: "Subject", sort: (a) => a.subject, render: (a) => <div className="min-w-0 max-w-[420px]"><div className={cn("truncate font-medium", a.completed_at ? "text-sand-400 line-through" : "text-sand-900")}>{a.subject}</div>{a.body && <div className="truncate text-[12px] leading-4 text-sand-500">{a.body}</div>}</div> },
    ...
    { key: "due", header: tab === "recent" ? "When" : "Due", width: tab === "recent" ? "180px" : "120px", sort: (a) => (tab === "recent" ? a.occurred_at : a.due_at) ?? "", render: (a) => tab === "recent" ? <span className="num whitespace-nowrap">{fmtDateTime(a.occurred_at)}</span> : a.due_at ? <span className={cn("num whitespace-nowrap", overdue(a) && "font-medium text-danger-600")}>{fmtDate(a.due_at)}</span> : <Dash /> },
