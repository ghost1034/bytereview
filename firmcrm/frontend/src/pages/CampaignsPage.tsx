...
import { Badge, Button, Card, DL, Empty, PageHeader, Select, Spinner, statusTone } from "@/components/ui";
...
import { usePracticeAreas, useUsers, opt, strOpts } from "@/lib/hooks";
import { CAMPAIGN_KINDS, CAMPAIGN_STATUSES, MEMBER_STATUSES } from "@/lib/options";
import { fmtDate, money, pct, titleCase } from "@/lib/format";
import { useAuth } from "@/lib/auth";
import { ArchivedChip, Dash, FilterToggle, NameCell, ResultCount, SearchInput, cellCount, cellMoney } from "@/components/ui/cells";

const Money = ({ n }: { n: number | null | undefined }) => (n ? <span className="num font-medium">{money(n)}</span> : <Dash />);
...
export default function CampaignsPage() {
  ...
  return (
    <div>
      ...
      <div className={sel ? "grid grid-cols-5 gap-4" : ""}>
        <div className={sel ? "col-span-3 card overflow-hidden self-start" : "card overflow-hidden"}><DataTable rows={camps.data} columns={visibleCols} loading={camps.isLoading} twoLine onRowClick={setSelected} sort={sorting.sort} onSortChange={sorting.onSortChange} empty="No campaigns yet" /></div>
        {sel && (
          <div className="col-span-2 space-y-4">
            ...
            <Card title={<>Members<span className="mono ml-1.5 text-[11px] text-sand-500">{members.data?.length ?? 0}</span></>} padded={false}>
              <div className="relative border-b border-sand-150 p-3">
                ...
                {addQ.length >= 2 && candidates.data && (
                  <div className="absolute top-full right-3 left-3 z-dropdown mt-1.5 overflow-hidden rounded-lg border border-sand-150 bg-sand-0 shadow-menu">
                    {candidates.data.map((c) => <button key={c.id} type="button" disabled={add.isPending} onClick={() => add.mutate({ id: c.id, full_name: c.full_name })} className="flex h-9 w-full items-center justify-between px-3 text-left text-[13px] hover:bg-sand-50 disabled:opacity-50"><span className="truncate">{c.full_name}</span><span className="ml-3 truncate text-[12px] text-sand-500">{c.account_name}</span></button>)}
                    {!candidates.data.length && <div className="px-3 py-2 text-[12px] text-sand-500">No contacts match</div>}
                  </div>)}
              </div>
