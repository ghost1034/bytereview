...
import { accountsApi, contactsApi, dataApi } from "@/api";
...
import { CONTACT_ROLES, LIFECYCLES } from "@/lib/options";
import { titleCase } from "@/lib/format";
import { ArchivedChip, FilterToggle, NameCell, ResultCount, SearchInput, cellDate, cellText } from "@/components/ui/cells";
...
export default function ContactsPage() {
  ...
  const cols: Column<Contact>[] = [
    ...
    { key: "email", header: "Email", sort: (c) => c.email ?? "", render: (c) => c.email ? <a href={`mailto:${c.email}`} onClick={(e) => e.stopPropagation()} className="block truncate whitespace-nowrap text-sand-900 hover:text-accent-600" title={c.email}>{c.email}</a> : cellText(null) },
