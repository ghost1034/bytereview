...
import { adminApi, refApi, usersApi, wallsApi } from "@/api";
import { Link } from "react-router-dom";
import type { Pipeline, PracticeArea, Stage, User, Wall } from "@/api/types";
import { Badge, Button, Card, Empty, Field, FieldError, Input, Modal, OverflowMenu, PageHeader, Select, Spinner, Tabs, type MenuItem } from "@/components/ui";
import { DataTable, type Column } from "@/components/ui/DataTable";
import { FormModal, type FieldDef, type FormValues } from "@/components/ui/Form";
import { useConfirm } from "@/components/ui/Confirm";
import { useToast } from "@/components/ui/Toast";
import { Pagination, usePager } from "@/components/ui/Pagination";
import { usePipelines, usePracticeAreas, opt, strOpts } from "@/lib/hooks";
import { DISCIPLINES, ROLES } from "@/lib/options";
import { fmtDateTime, titleCase } from "@/lib/format";
import { useAuth } from "@/lib/auth";
import { Dash, FilterToggle, NameCell } from "@/components/ui/cells";

const Note = ({ children, top = false }: { children: React.ReactNode; top?: boolean }) => (
  <div className={top ? "border-b border-sand-150 px-5 py-2.5 text-[12px] leading-4 text-sand-500" : "border-t border-sand-150 px-5 py-2.5 text-[12px] leading-4 text-sand-500"}>{children}</div>
);

type AuditRow = Awaited<ReturnType<typeof adminApi.audit>>["items"][number];

/** Same rule the API enforces; shown inline instead of a server error toast. */
const PASSWORD_RULE = "At least 12 characters with letters and digits";
const passwordError = (v: unknown) => {
  if (typeof v !== "string" || v === "") return null;
  if (v.length < 12 || !/[A-Za-z]/.test(v) || !/\d/.test(v)) return `${PASSWORD_RULE}.`;
  return null;
};
...
export default function AdminPage() {
  ...
  const addMember = useMutation({ mutationFn: ({ wall, uid }: { wall: Wall; uid: number }) => wallsApi.addMember(wall.id, uid), onSuccess: (_, v) => { invWalls(); setAddingTo(null); setAddUserId(""); toast(v.uid === user?.id ? "You were added to the wall" : "Member added"); }, onError: error });
  ...
  const [newUser, setNewUser] = useState(false); const [editUser, setEditUser] = useState<User | null>(null);
  ...
  const userFields = (create: boolean): FieldDef[] => [
    { name: "full_name", label: "Full name", required: true }, { name: "email", label: "Email", type: "email", required: true },
    { name: "role", label: "Role", type: "select", options: strOpts(ROLES) }, { name: "title", label: "Title" },
    { name: "practice_area_id", label: "Practice area", type: "select", options: opt(activePas.data, (p) => p.name) },
    { name: "password", label: create ? "Temporary password" : "Reset password", type: "password", required: create, hint: create ? `${PASSWORD_RULE}. The user must change it at first sign-in.` : `Leave blank to keep the current password. ${PASSWORD_RULE}.`, validate: passwordError },
  ...
  const auditCols: Column<AuditRow>[] = [
    { key: "at", header: "When", width: "170px", nowrap: true, render: (r) => <span className="num text-sand-600">{fmtDateTime(r.at)}</span> },
    { key: "actor", header: "Actor", width: "160px", nowrap: true, hideBelow: 1180, render: (r) => r.actor_name ?? <span className="text-sand-500">system</span> },
    ...
    { key: "entity", header: "Entity", width: "170px", nowrap: true, render: (r) => <span className="text-sand-700">{titleCase(r.entity_type)} <span className="mono text-sand-500">#{r.entity_id}</span></span> },
    { key: "diff", header: "Before → after", maxWidth: "240px", render: (r) => <span className="mono block max-w-[240px] truncate text-sand-600" title={`${r.before_json ?? ""} → ${r.after_json ?? ""}${r.note ? ` (${r.note})` : ""}`}>{r.before_json ? `${r.before_json} → ` : ""}{r.after_json}{r.note ? ` (${r.note})` : ""}</span> },
  ];
  return (
    <div>
      ...
      <div className="mt-5 space-y-4">
        {tab === "users" && <Card title="Users" padded={false} actions={isAdmin && <Button size="sm" onClick={() => setNewUser(true)}><Plus size={12} />Add user</Button>}>
          {users.isLoading ? <Spinner /> : <table className="tbl"><thead><tr><th style={{ width: 260 }}>Name</th><th>Email</th><th style={{ width: 120 }}>Role</th><th style={{ width: 200 }} className="max-[1279px]:hidden">Practice area</th><th style={{ width: 180 }} className="max-[1179px]:hidden">Last sign-in</th><th style={{ width: 110 }}>Status</th></tr></thead>
            <tbody>{users.data?.map((u) => <tr key={u.id} className={isAdmin ? "clickable row-2line" : "row-2line"} tabIndex={isAdmin ? 0 : undefined} onClick={() => isAdmin && setEditUser(u)} onKeyDown={(e) => { if (isAdmin && e.target === e.currentTarget && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); setEditUser(u); } }}>
              <td><NameCell name={u.full_name} sub={u.title} max={260} /></td><td><span className="block max-w-[260px] truncate whitespace-nowrap text-sand-600" title={u.email}>{u.email}</span></td><td><Badge>{titleCase(u.role)}</Badge></td>
              <td className="whitespace-nowrap max-[1279px]:hidden">{pas.data?.find((p) => p.id === u.practice_area_id)?.name ?? <Dash />}</td><td className="num whitespace-nowrap text-sand-600 max-[1179px]:hidden">{u.last_login_at ? fmtDateTime(u.last_login_at) : <Dash />}</td><td>{active(u.is_active)}</td></tr>)}</tbody></table>}
        </Card>}
        {tab === "practice" && <Card title="Practice areas" padded={false} actions={isAdmin && <Button size="sm" onClick={() => setNewPa(true)}><Plus size={12} />Add practice area</Button>}>
          {pas.isLoading ? <Spinner /> : <table className="tbl"><thead><tr><th style={{ width: 300 }}>Name</th><th style={{ width: 160 }}>Discipline</th><th>Clearance gate</th><th style={{ width: 110 }}>Status</th></tr></thead>
            <tbody>{pas.data?.map((p) => <tr key={p.id} className={isAdmin ? "clickable" : ""} tabIndex={isAdmin ? 0 : undefined} onClick={() => isAdmin && setEditPa(p)} onKeyDown={(e) => { if (isAdmin && e.target === e.currentTarget && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); setEditPa(p); } }}>
              ...
              <td>{p.clearance_type ? <Badge dot tone="warn">{titleCase(p.clearance_type)} check before Closed Won</Badge> : <span className="text-sand-400">None</span>}</td><td>{active(p.is_active)}</td></tr>)}</tbody></table>}
        </Card>}
        ...
        {tab === "walls" && <Card title="Ethical walls" padded={false} actions={<FilterToggle checked={inactiveWalls} onChange={setInactiveWalls}>Show lifted</FilterToggle>}>
          ...
          {walls.isLoading ? <Spinner /> : !walls.data?.items.length ? <Empty title="No ethical walls" hint={inactiveWalls ? "No walls have been raised." : "No active walls. Lifted walls are hidden."} /> : <table className="tbl"><thead><tr><th style={{ width: 280 }}>Record</th><th>Reason</th><th style={{ width: 240 }}>Members</th><th style={{ width: 200 }}>Raised by</th><th style={{ width: 200 }}>Status</th>{canManageWalls && <th style={{ width: 56 }}></th>}</tr></thead>
            <tbody>{walls.data.items.map((w) => <tr key={w.id}>
              <td><Link to={`/${w.entity_type === "account" ? "accounts" : "opportunities"}/${w.entity_id}`} className="font-medium">{w.entity_name ?? `${w.entity_type} #${w.entity_id}`}</Link><div className="text-[12px] leading-4 text-sand-500">{titleCase(w.entity_type)}</div></td>
              <td className="max-w-[360px] py-2 whitespace-normal">{w.reason}</td><td className="whitespace-normal text-sand-700">{w.members.map((m) => m.full_name).join(", ")}</td>
              <td>{w.created_by_name ?? <Dash />}<div className="num text-[12px] leading-4 text-sand-500">{fmtDateTime(w.created_at)}</div></td>
      ...
      <FormModal open={newUser} onClose={() => setNewUser(false)} title="Add user" fields={userFields(true)} initial={{ role: "staff", is_active: true }} submitLabel="Create"
        onSubmit={async (v) => { await usersApi.create(v as Partial<User> & { password: string }); qc.invalidateQueries({ queryKey: ["users"] }); toast("User created"); }} />
      {editUser && <FormModal open onClose={() => setEditUser(null)} title={`Edit ${editUser.full_name}`} fields={userFields(false).filter((f) => f.name !== "email")} initial={editUser as unknown as FormValues}
        onSubmit={async (v) => { const { password, full_name, role, title, practice_area_id, is_active } = v as Record<string, unknown>; await usersApi.update(editUser.id, { full_name, role, title, practice_area_id, is_active, ...(password ? { password } : {}) } as Partial<User> & { password?: string }); qc.invalidateQueries({ queryKey: ["users"] }); toast("User updated"); }} />}
      ...
      <Modal open={!!addingTo} onClose={() => setAddingTo(null)} title={`Add member · ${addingTo?.entity_name ?? ""}`}
             footer={<><Button onClick={() => setAddingTo(null)}>Cancel</Button><Button variant="primary" disabled={!addUserId || addMember.isPending} onClick={() => addingTo && addMember.mutate({ wall: addingTo, uid: Number(addUserId) })}><UserPlus size={12} />Add</Button></>}>
