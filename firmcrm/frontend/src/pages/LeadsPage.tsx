...
import { accountsApi, campaignsApi, dataApi, leadsApi } from "@/api";
import { Pagination, usePager } from "@/components/ui/Pagination";
import { useAuth } from "@/lib/auth";
import type { Lead } from "@/api/types";
import { Badge, Button, Empty, Field, Input, Modal, OverflowMenu, PageHeader, Select, cn, statusTone, type MenuItem } from "@/components/ui";
import { DataTable, useServerSort, type Column } from "@/components/ui/DataTable";
import { FormModal, MoneyInput, type FieldDef, type FormValues } from "@/components/ui/Form";
import { useConfirm, useReasonPrompt } from "@/components/ui/Confirm";
import { useToast } from "@/components/ui/Toast";
import { usePracticeAreas, useUsers, opt, strOpts } from "@/lib/hooks";
import { LEAD_SOURCES, LEAD_STATUSES } from "@/lib/options";
import { titleCase } from "@/lib/format";
import { ArchivedChip, FilterToggle, NameCell, ResultCount, SearchInput, cellMoney, cellText } from "@/components/ui/cells";
    ...
    <Modal open onClose={onClose} title={`Convert lead · ${lead.first_name} ${lead.last_name}`} footer={<><Button onClick={onClose}>Cancel</Button><Button variant="primary" disabled={m.isPending || (mode === "existing" && !existingId)} onClick={() => m.mutate()}>{m.isPending ? "Converting…" : "Convert"}</Button></>}>
      <div className="space-y-4">
        <div>
          ...
          <div className="space-y-2 text-[13px] text-sand-900">
          ...
          {mode === "new" && matches.data && matches.data.length > 0 && (
            <div className="mt-2 flex items-start gap-1.5 text-[12px] leading-4 text-warn-700">
          ...
          {adverse.length > 0 && (
            <div className="mt-2 flex items-start gap-1.5 text-[12px] leading-4 text-warn-700" role="status">
              <AlertTriangle size={14} className="mt-px shrink-0 text-warn-600" aria-hidden />
        ...
        {mode === "existing" && (
          <div className="space-y-2">
            ...
            <div className="max-h-[180px] overflow-auto rounded-md border border-sand-150">
              {(matches.data ?? []).map((a) => (
                <label key={a.id} className={cn("flex h-9 cursor-pointer items-center gap-2 px-3 text-[13px] hover:bg-sand-25", existingId === a.id && "bg-accent-50")}>
              ...
              {accountQ.length >= 2 && matches.data && !matches.data.length && <div className="px-3 py-2 text-[12px] text-sand-500">No accounts match</div>}
              {accountQ.length < 2 && <div className="px-3 py-2 text-[12px] text-sand-500">Type at least two characters to search</div>}
            </div>
          </div>)}
        <label className="flex items-center gap-2 text-[13px] text-sand-900"><input type="checkbox" checked={createOpp} onChange={(e) => setCreateOpp(e.target.checked)} />Also create an opportunity</label>
