...
import type { TimelineEvent } from "@/components/crm/ActivityTimeline";
import { conflictsApi, oppsApi } from "@/api";
import type { Opportunity, Stage } from "@/api/types";
import { Badge, Button, Card, DL, Drawer, Field, Input, OverflowMenu, PageHeader, Select, Spinner, Textarea, cn, statusTone, type MenuItem } from "@/components/ui";
import { FormModal, type FormValues } from "@/components/ui/Form";
import { useToast } from "@/components/ui/Toast";
import { useConfirm } from "@/components/ui/Confirm";
import { ActivityTimeline } from "@/components/crm/ActivityTimeline";
import { WallPanel, useWall } from "@/components/crm/WallPanel";
import { StageRail } from "@/components/crm/StageRail";
import { isPastDue } from "@/components/crm/KanbanCard";
import { ClearanceList } from "./ClearancePage";
import { useOpportunityFields } from "./OpportunitiesPage";
import { usePipelines, strOpts } from "@/lib/hooks";
import { EL_STATUSES, INDEPENDENCE_QUESTIONS, LOST_REASONS } from "@/lib/options";
  ...
  return (
    <Drawer open onClose={onClose} title="Mark lost" footer={<><Button onClick={onClose}>Cancel</Button><Button variant="danger-solid" onClick={() => m.mutate()} disabled={m.isPending}>Mark lost</Button></>}>
      <div className="space-y-4">
        <p className="text-[13px] leading-5 text-sand-600"><span className="font-medium text-sand-900">{opp.name}</span> will be closed as lost. The reason is recorded in the stage history and win/loss reporting.</p>
      ...
      <div className="space-y-4">
        ...
        {type === "independence" && (
          <div>
            ...
            <div className="space-y-2 rounded-md border border-sand-150 p-3">{INDEPENDENCE_QUESTIONS.map((q) => <label key={q.key} className="flex items-start gap-2.5 text-[13px] leading-5 text-sand-900"><input type="checkbox" className="mt-[3px] shrink-0" checked={att[q.key]} onChange={(e) => setAtt({ ...att, [q.key]: e.target.checked })} />{q.label}</label>)}</div>
            <div className="mt-1.5 text-[12px] leading-4 text-sand-500">Any “yes” routes the check to partner review.</div>
          </div>)}
      </div>
    </Drawer>
  );
...
function Fact({ label, children, sub, className }: { label: string; children: ReactNode; sub?: ReactNode; className?: string }) {
  return (
    <div className={cn("min-w-0 border-r border-sand-100 px-5 py-3.5 last:border-r-0", className)}>
      <div className="text-[12px] leading-4 font-medium text-sand-600">{label}</div>
      <div className="mt-1 min-w-0">{children}</div>
      {sub && <div className="mt-0.5 text-[12px] leading-4 text-sand-500">{sub}</div>}
    </div>
  );
}
const inlineCtl = "h-8 rounded-md border border-transparent bg-transparent text-sand-900 transition-[border-color,box-shadow] duration-[120ms] hover:border-sand-200 focus:border-accent-600 focus:shadow-[0_0_0_3px_var(--color-accent-100)] focus:outline-none disabled:cursor-default disabled:hover:border-transparent";
...
export default function OpportunityDetailPage() {
  ...
  const dot = <span className="mx-1.5 text-sand-300">·</span>;
  ...
  const timelineEvents: TimelineEvent[] = [
    ...(hist.data ?? []).map((h) => ({
      ...
      title: <><span className="font-medium">{h.changed_by_name ?? "System"}</span> <span className="text-sand-500">{h.from_stage_name ? "moved to" : "created in"}</span> {h.to_stage_name}</>,
    ...
    ...(checks.data ?? []).flatMap((c) => [
      { id: `c${c.id}`, at: c.created_at, kind: "clearance" as const,
        title: <><span className="font-medium">{c.requested_by_name ?? "Someone"}</span> <span className="text-sand-500">ran {c.check_type === "independence" ? "an independence" : "a conflict"} check</span> <span className="mono text-sand-500">#{c.id}</span></>,
      ...
      ...(c.resolved_at && c.status !== "pending" ? [{ id: `r${c.id}`, at: c.resolved_at, kind: "clearance" as const,
        title: <><span className="font-medium">{c.resolved_by_name ?? "Someone"}</span> <span className="text-sand-500">resolved check</span> <span className="mono text-sand-500">#{c.id}</span> <span className="text-sand-300">·</span> {titleCase(c.status)}</>,
  ...
  return (
    <div className="max-w-[1344px]">
      ...
      {/* Key facts (§6.4) */}
      <div className="card mb-4 grid grid-cols-6 overflow-hidden">
        <Fact label="Amount" sub={<>{titleCase(o.fee_type)}{o.is_recurring && o.fee_type !== "recurring" ? " · recurring" : ""}</>}>
          <div className="text-[24px] leading-7 font-semibold tracking-[-0.02em] text-sand-900 num">{money(o.amount)}</div>
        </Fact>
        <Fact label="Probability" sub={<>→ <span className="num">{money(o.weighted_amount)}</span> weighted</>}>
          <div className="-ml-1.5 flex items-center">
            ...
            <span className="text-[18px] leading-6 font-semibold text-sand-500">%</span>
          </div>
        </Fact>
        <Fact label="Expected close" sub={<span className="num">{o.days_in_stage} days in stage</span>}>
          <div className={cn("text-[20px] leading-7 font-semibold tracking-[-0.015em] num", pastDue ? "text-danger-600" : "text-sand-900")} title={pastDue ? "Expected close date has passed" : undefined}>{fmtDate(o.expected_close)}</div>
        </Fact>
        <Fact label="Engagement letter" sub={elOk ? "Gate satisfied" : isOpen ? "Required before Closed Won" : undefined}>
          <div className="relative -ml-1.5 inline-flex items-center">
            <select value={o.engagement_letter_status} disabled={!isOpen} onChange={(e) => quick.mutate({ engagement_letter_status: e.target.value })} aria-label="Engagement letter status"
            ...
            {isOpen && <ChevronDown size={14} className="pointer-events-none absolute right-2 text-sand-500" />}
          </div>
        </Fact>
        <Fact label="Owner">
          <div className="truncate text-[20px] leading-7 font-semibold tracking-[-0.015em] text-sand-900" title={o.owner_name ?? undefined}>{o.owner_name ?? <span className="text-sand-300">—</span>}</div>
        </Fact>
        <Fact label="Originating partner">
          <div className="truncate text-[20px] leading-7 font-semibold tracking-[-0.015em] text-sand-900" title={o.originating_partner_name ?? undefined}>{o.originating_partner_name ?? <span className="text-sand-300">—</span>}</div>
        </Fact>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-4 space-y-4">
          <Card tourId="clearance-card" title={<span className="inline-flex items-center gap-1.5"><ShieldCheck size={14} className="text-sand-500" />{clearanceLabel} clearance</span>}
                actions={isOpen && <Button size="sm" onClick={() => setChecking(true)}>Run check</Button>}>
            {!o.clearance_type ? <div className="mb-3 text-[12px] leading-4 text-sand-500">This practice area has no mandatory clearance gate. You may still run an ad-hoc conflict check.</div> : (
              <div className="mb-3 flex items-start gap-2 text-[13px] leading-5">
                ...
                {o.clearance_status === "waived" && <ShieldOff size={12} className="mt-1 shrink-0 text-success-600" aria-label="Waived" />}
                <span className="text-sand-600">{gateOk ? "Gate satisfied." : o.clearance_status ? "Closed Won is blocked until cleared or waived." : "No check run yet. Required before Closed Won."}</span>
              </div>)}
...
function DetailSkeleton() {
  return (
    <div className="max-w-[1344px]" aria-busy="true" aria-label="Loading opportunity">
      ...
      <div className="card mb-4 grid grid-cols-6">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="space-y-2.5 border-r border-sand-100 px-5 py-3.5 last:border-r-0"><span className="skeleton w-2/5" /><span className="skeleton h-4 w-4/5" /></div>)}</div>
      <div className="grid grid-cols-12 gap-4"><div className="col-span-8 card h-[240px]"><Spinner /></div><div className="col-span-4 card h-[160px]"><Spinner /></div></div>
    </div>
  );
}
