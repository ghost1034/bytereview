...
export function FactsGrid({ facts, className }: { facts: Fact[]; className?: string }) {
  return (
    <div className={cn("card mb-6 overflow-hidden", className)}>
      <div className={cn("-mr-px -mb-px grid", facts.length === 6 && "grid-cols-6 max-[1279px]:grid-cols-3")} style={facts.length === 6 ? undefined : { gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
        {facts.map((f) => (
          <div key={f.label} className="min-w-0 border-r border-b border-sand-100 px-5 py-3.5" title={typeof f.value === "string" ? f.value : undefined}>
            <div className="text-[12px] leading-4 font-medium text-sand-600">{f.label}</div>
            <div className={cn("mt-1 font-semibold text-sand-900 num", f.size === "text" ? "break-words text-[20px] leading-7 tracking-[-0.015em]" : "whitespace-nowrap text-[24px] leading-7 tracking-[-0.02em] max-[1279px]:text-[20px]")}>{f.value ?? <Dash />}</div>
            {f.sub != null && <div className="mt-0.5 break-words text-[12px] leading-4 text-sand-500">{f.sub}</div>}
          </div>
        ))}
      </div>
    </div>
...
export function NotFound({ what, backTo, backLabel, hint = "It may have been removed, or access is restricted by an ethical wall." }: { what: string; backTo: string; backLabel: string; hint?: string }) {
  return (
    <div className="card max-w-[480px] p-6">
      ...
      <div className="mt-1 text-[12px] leading-4 text-sand-500">{hint}</div>
