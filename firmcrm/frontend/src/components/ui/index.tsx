...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button({ variant = "secondary", size = "md", className, ...p }, ref) {
  const base = "inline-flex items-center justify-center gap-1.5 rounded-md border font-medium whitespace-nowrap transition-colors duration-[120ms] disabled:opacity-50 disabled:cursor-not-allowed";
  const sizes = { sm: "h-7 px-2.5 text-[12px] leading-4", md: "h-8 px-3 text-[13px] leading-5", lg: "h-9 px-3.5 text-[13px] leading-5" };
  const variants = {
    primary: "bg-accent-600 text-white border-accent-600 hover:bg-accent-700 hover:border-accent-700 active:bg-accent-800",
    secondary: "bg-sand-0 text-sand-900 border-sand-200 hover:bg-sand-25 hover:border-sand-300 active:bg-sand-50",
    ghost: "bg-transparent text-sand-700 border-transparent hover:bg-sand-100 hover:text-sand-900 active:bg-sand-150",
    danger: "bg-sand-0 text-danger-600 border-sand-200 hover:bg-danger-50 hover:border-danger-200 active:text-danger-700",
    "danger-solid": "bg-danger-600 text-white border-danger-600 hover:bg-danger-700 hover:border-danger-700 active:bg-danger-700",
  };
...
export function Field({ label, children, hint, className, error, errorId }: { label: string; children: ReactNode; hint?: string; className?: string; error?: string | null; errorId?: string }) {
  ...
  return (
    <label className={cn("block", className)}>
      <span className="label">{text}{required && <span className="ml-1 text-danger-600">*</span>}</span>
      {children}
      {error ? <FieldError id={errorId}>{error}</FieldError> : hint && <span className="mt-1 block text-[12px] leading-4 text-sand-500">{hint}</span>}
    </label>
  );
}
...
const PILL: Record<string, string> = {
  neutral: "bg-sand-100 border-sand-150 text-sand-700",
...
const DOT: Record<string, string> = { neutral: "bg-sand-400", success: "bg-success-600", danger: "bg-danger-600", warn: "bg-warn-600", info: "bg-info-600", accent: "bg-accent-600" };
export function Badge({ tone = "slate", dot = false, children, className, title }: { tone?: Tone; dot?: boolean; children: ReactNode; className?: string; title?: string }) {
  ...
  if (dot) {
    return (
      <span title={title} className={cn("inline-flex items-center gap-1.5 text-[12px] leading-4 font-medium text-sand-900 whitespace-nowrap", className)}>
    ...
    <section className={cn("card", !padded && "overflow-hidden", className)} data-tour={tourId}>
      {hasHeader && (
        <header className="flex items-center justify-between gap-4 px-5 pt-3.5 pb-3">
          <h3 className="min-w-0 truncate text-[15px] leading-[22px] font-semibold tracking-[-0.01em] text-sand-900">{title}</h3>
...
export function Modal({ open, onClose, title, children, footer, width, size = "default", initialFocus, hideClose }: OverlayProps & { width?: string; size?: "default" | "wide" }) {
  ...
  return (
    <div className="fade-in fixed inset-0 z-modal flex items-start justify-center overflow-y-auto bg-[rgba(26,25,22,0.32)] p-4 pt-[8vh]" onMouseDown={onClose}>
      <div ref={panel} className={cn("modal-in w-full rounded-xl border border-sand-150 bg-sand-0 shadow-modal outline-none", w)} onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        <header className="flex items-center justify-between gap-4 border-b border-sand-150 px-5 py-4">
          <h2 id={titleId} className="min-w-0 truncate text-[15px] leading-[22px] font-semibold tracking-[-0.01em] text-sand-900">{title}</h2>
          {!hideClose && <button ref={closeBtn} type="button" onClick={onClose} aria-label="Close" className="grid h-7 w-7 shrink-0 place-items-center rounded-md text-sand-500 hover:bg-sand-100 hover:text-sand-900"><X size={16} /></button>}
        </header>
        <div className="px-5 py-5">{children}</div>
        {footer && <footer className="flex justify-end gap-2 rounded-b-xl border-t border-sand-150 bg-sand-25 px-5 py-3">{footer}</footer>}
      </div>
    </div>
  );
}
 ...
 * 480px wide, full height, hairline left border, shadow-modal, 200ms translateX(16px→0). Same props and focus handling as Modal.
 */
export function Drawer({ open, onClose, title, children, footer, initialFocus, hideClose, width = "w-[480px]" }: OverlayProps & { width?: string }) {
  ...
  if (!open) return null;
  return (
    <div className="fade-in fixed inset-0 z-drawer flex justify-end bg-[rgba(26,25,22,0.32)]" onMouseDown={onClose}>
      <div ref={panel} className={cn("drawer-in flex h-full max-w-full flex-col border-l border-sand-150 bg-sand-0 shadow-modal outline-none", width)} onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        <header className="flex shrink-0 items-center justify-between gap-4 border-b border-sand-150 px-5 py-4">
          <h2 id={titleId} className="min-w-0 truncate text-[15px] leading-[22px] font-semibold tracking-[-0.01em] text-sand-900">{title}</h2>
          {!hideClose && <button ref={closeBtn} type="button" onClick={onClose} aria-label="Close" className="grid h-7 w-7 shrink-0 place-items-center rounded-md text-sand-500 hover:bg-sand-100 hover:text-sand-900"><X size={16} /></button>}
        </header>
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">{children}</div>
        {footer && <footer className="flex shrink-0 justify-end gap-2 border-t border-sand-150 bg-sand-25 px-5 py-3">{footer}</footer>}
      </div>
    </div>
  );
}
 ...
 * shadow-menu, 36px items, hover sand-50. Arrow keys move, Enter selects, Escape closes, outside click closes.
  ...
  return (
    <div ref={root} className={cn("relative inline-flex", className)} data-open={open || undefined} onKeyDown={onKey}>
      <button ref={btn} type="button" aria-label={label} title={label} aria-haspopup="menu" aria-expanded={open} aria-controls={open ? menuId : undefined}
              ...
              className={cn("grid place-items-center rounded-md border border-transparent text-sand-700 transition-colors duration-[120ms] hover:bg-sand-100 hover:text-sand-900 active:bg-sand-150", size === "sm" ? "h-7 w-7" : "h-8 w-8", open && "bg-sand-100 text-sand-900", buttonClassName)}>
        <MoreHorizontal size={16} />
      </button>
      {open && (
        <div ref={list} id={menuId} role="menu" aria-label={label}
             className={cn("menu-in absolute z-dropdown min-w-[180px] rounded-lg border border-sand-150 bg-sand-0 py-1 shadow-menu", align === "end" ? "right-0" : "left-0", side === "bottom" ? "top-full mt-1.5" : "bottom-full mb-1.5")}>
          {items.map((it, idx) => (
            <button key={idx} type="button" role="menuitem" disabled={it.disabled} onClick={() => { close(false); btn.current?.focus(); it.onSelect(); }}
                    className={cn("flex h-9 w-full items-center gap-2.5 px-3 text-left text-[13px] leading-5 whitespace-nowrap outline-none transition-colors duration-[120ms] hover:bg-sand-50 focus-visible:bg-sand-50 disabled:cursor-not-allowed disabled:opacity-50",
                      it.tone === "danger" ? "text-danger-600" : "text-sand-900")}>
              {it.icon && <span className={cn("grid w-4 shrink-0 place-items-center [&_svg]:h-3.5 [&_svg]:w-3.5", it.tone === "danger" ? "text-danger-600" : "text-sand-500")}>{it.icon}</span>}
              {it.label}
            </button>
          ))}
        </div>
...
export function Empty({ title = "Nothing here yet", hint, action, icon }: { title?: string; hint?: string; action?: ReactNode; icon?: ReactNode }) {
  return (
    <div className="mx-auto flex max-w-[360px] flex-col items-center justify-center px-6 py-12 text-center">
      <div className="grid h-8 w-8 place-items-center rounded-full bg-sand-100 text-sand-500">{icon ?? <Inbox size={16} />}</div>
      <div className="mt-3 text-[13px] font-medium text-sand-900">{title}</div>
      {hint && <div className="mt-1 text-[12px] leading-4 text-sand-500">{hint}</div>}
...
export function Sparkline({ data, tone = "accent", className }: { data: number[]; tone?: "accent" | "warn" | "success" | "danger"; className?: string }) {
  ...
  const stroke = { accent: "stroke-accent-600", warn: "stroke-warn-600", success: "stroke-success-600", danger: "stroke-danger-600" }[tone];
  const fill = { accent: "fill-accent-600", warn: "fill-warn-600", success: "fill-success-600", danger: "fill-danger-600" }[tone];
...
export function Stat({ label, value, sub, tone, delta, deltaGood, spark, className }: {
  ...
  const deltaCls = delta == null || delta === 0 ? "text-sand-500" : good ? "text-success-600" : "text-danger-600";
  ...
  return (
    <div className={cn("card flex min-h-[128px] flex-col px-5 pt-4 pb-3.5", className)}>
      <div className="text-[12px] leading-4 font-medium text-sand-600 whitespace-nowrap truncate">{label}</div>
      <div className={cn("mt-2 text-[24px] leading-7 font-semibold tracking-[-0.02em] num", tone === "warn" && "text-warn-700", tone === "good" && "text-success-700")}>{value}</div>
      {(delta != null || sub) && (
        <div className="mt-1.5 flex flex-wrap items-center gap-x-1.5 text-[12px] leading-4 text-sand-500">
          {delta != null && (
...
export function Tabs<T extends string>({ tabs, value, onChange }: { tabs: { key: T; label: string; count?: number }[]; value: T; onChange: (k: T) => void }) {
  return (
    <div className="flex gap-5 border-b border-sand-150" role="tablist">
      {tabs.map((t) => (
        <button key={t.key} type="button" role="tab" aria-selected={value === t.key} onClick={() => onChange(t.key)}
                className={cn("-mb-px h-9 border-b-2 px-0.5 text-[13px] font-medium transition-colors duration-[120ms]", value === t.key ? "border-sand-900 text-sand-900" : "border-transparent text-sand-600 hover:text-sand-900")}>
          {t.label}{t.count !== undefined && <span className="mono ml-1.5 text-[11px] text-sand-500">{t.count}</span>}
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0">
        <h1 className="flex items-center gap-2 text-[20px] leading-7 font-semibold tracking-[-0.015em] text-sand-900">{title}</h1>
        {subtitle && <div className="mt-1 text-[12px] leading-4 text-sand-500 [&_a]:text-sand-700 [&_a:hover]:text-sand-900">{subtitle}</div>}
      </div>
...
export function DL({ items, columns }: { items: { label: string; value: ReactNode; span?: number }[]; columns?: number }) {
  return (
    <dl className="grid gap-x-6 gap-y-4 text-[13px]" style={{ gridTemplateColumns: columns ? `repeat(${columns}, minmax(0, 1fr))` : "repeat(auto-fill, minmax(200px, 1fr))" }}>
      {items.map((i) => (
        <Fragment key={i.label}>
          <div className="min-w-0" style={i.span ? { gridColumn: `span ${i.span} / span ${i.span}` } : undefined}>
            <dt className="text-[12px] leading-4 font-medium text-sand-600">{i.label}</dt>
            <dd className="mt-0.5 min-w-0 break-words text-sand-900">{i.value ?? <span className="text-sand-300">—</span>}</dd>
          </div>
        </Fragment>
      ))}
    </dl>
