...
const fmtBound = (f: FieldDef, n: number) => (f.type === "money" ? `$${n.toLocaleString("en-US")}` : n.toLocaleString("en-US"));
...
export function TagInput({ value, onChange, placeholder, id, ...a11y }: { value: string[]; onChange: (v: string[]) => void; placeholder?: string; id?: string } & Record<string, unknown>) {
  ...
  return (
    <div className="field flex h-auto min-h-8 flex-wrap items-center gap-1 py-1" onClick={(e) => (e.currentTarget.querySelector("input") as HTMLInputElement | null)?.focus()}>
      {value.map((t) => (
        <span key={t} className="inline-flex h-5 items-center gap-1 rounded-full border border-sand-150 bg-sand-100 px-2 text-[11px] leading-[14px] font-medium text-sand-700">
          {t}<button type="button" aria-label={`Remove ${t}`} onClick={() => onChange(value.filter((x) => x !== t))} className="text-sand-500 hover:text-sand-900">×</button>
        </span>))}
      <input {...(a11y as object)} id={id} value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={value.length ? "" : placeholder ?? "Type and press Enter"}
             ...
             onBlur={commit} className="min-w-[120px] flex-1 bg-transparent outline-none placeholder:text-sand-400" />
    </div>
  );
}
...
export function MoneyInput({ value, onValueChange, className, onBlur, onFocus, ...p }: Omit<InputHTMLAttributes<HTMLInputElement>, "value" | "onChange" | "type"> & { value: number | null | undefined; onValueChange: (n: number | null) => void }) {
  ...
  return (
    <span className="relative block">
      <span className="field-prefix" aria-hidden>$</span>
      <Input {...p} type="text" inputMode="decimal" autoComplete="off" value={text} className={cn("!pl-6 text-right num", className)}
    ...
    <div className="grid grid-cols-2 gap-4">
      {fields.map((f) => {
        ...
        if (f.type === "checkbox") return (
          <label key={f.name} className={`flex items-center gap-2 self-end pb-2 text-[13px] text-sand-900 ${cls}`}>
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} width={width} size={size}>
      <form onSubmit={submit} noValidate>
        ...
        <div className="-mx-5 -mb-5 mt-5 flex justify-end gap-2 rounded-b-xl border-t border-sand-150 bg-sand-25 px-5 py-3">
