      ...
      <table className={cn("tbl", layout === "fixed" && "table-fixed")}>
        <thead><tr>{columns.map((c) => (
          <th key={c.key} style={{ width: c.width }} aria-sort={sort?.key === c.key ? (sort.dir === "asc" ? "ascending" : "descending") : undefined}
              className={cn(c.align === "right" && "!text-right", c.sort && "cursor-pointer select-none hover:text-sand-900", sort?.key === c.key && "text-sand-900", hideCls(c))}
        ...
        <tbody>{sorted.map((r) => (
          <tr key={r.id} className={cn(onRowClick && "clickable focus-visible:outline-2 focus-visible:outline-accent-600 focus-visible:-outline-offset-2", twoLine && "row-2line")}
