...
export const yAxisMoneyProps = { axisLine: false, tickLine: false, tick: tickStyle, width: 48, tickFormatter: (v: number) => money(v, true) } as const;
...
export const tooltipProps = {
  ...
  contentStyle: { background: "#1A1916", border: "none", borderRadius: 8, padding: "8px 10px", boxShadow: "var(--shadow-menu)" } as CSSProperties,
...
export const moneyTooltipFormatter = (v: number | string) => money(Number(v));
...
export function ChartLegend({ items, className }: { items: { label: string; color: string }[]; className?: string }) {
  return (
    <div className={cn("flex items-center gap-4 text-[12px] leading-4 text-sand-600", className)} aria-hidden>
      {items.map((i) => (
...
export function WinLossBar({ won, lost, className }: { won: number; lost: number; className?: string }) {
  ...
  return (
    <div className={cn("flex h-2 w-full overflow-hidden rounded-full", total > 0 ? "bg-danger-200" : "bg-sand-150", className)} role="img" aria-label={total > 0 ? `${won} won, ${lost} lost` : "No closed opportunities"}>
      <i className="block h-full bg-success-600" style={{ width: `${share}%` }} />
    </div>
  );
}
