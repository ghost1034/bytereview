...
export const authApi = {
  login: (email: string, password: string) => post<T.TokenPair>("/auth/login", { email, password }),
  me: () => get<T.User>("/auth/me"),
  logout: (refresh_token: string | null) => post<void>("/auth/logout", { refresh_token }),
  logoutAll: () => post<{ revoked: number }>("/auth/logout-all"),
  changePassword: (current_password: string, new_password: string) => post<T.TokenPair>("/auth/change-password", { current_password, new_password }),
  sessions: () => get<T.SessionInfo[]>("/auth/sessions"),
  revokeSession: (id: number) => del(`/auth/sessions/${id}`),
};
export const usersApi = {
  list: (include_inactive = false) => get<T.User[]>("/users", { include_inactive }),
  create: (body: Partial<T.User> & { password: string }) => post<T.User>("/users", body),
  update: (id: number, body: Partial<T.User> & { password?: string }) => patch<T.User>(`/users/${id}`, body),
};
export const refApi = {
...
export const wallsApi = {
  ...
  create: (b: { entity_type: "account" | "opportunity"; entity_id: number; reason: string; member_ids: number[] }) => post<T.Wall>("/walls", b),
  addMember: (id: number, user_id: number) => post<T.Wall>(`/walls/${id}/members`, { user_id }),
  removeMember: (id: number, user_id: number) => del<T.Wall>(`/walls/${id}/members/${user_id}`),
