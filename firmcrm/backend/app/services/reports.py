...
from app.services.opportunities import is_stale
...
def pipeline_summary(db: Session, pipeline_id: int | None = None) -> dict:
    ...
    for o in opps:
        ...
        b["stale"] += 1 if is_stale(o) else 0
    ...
    return {
        ...
        "stale_count": sum(1 for o in opps if is_stale(o)),
    }
...
def referral_sources(db: Session) -> list[dict]:
    ...
    for o in db.scalars(select(Opportunity)).all():
        ...
        if o.referral_contact_id and o.referral_contact_id in contacts:
            ...
            key, name, org = f"a{o.referral_account_id}", accounts.get(o.referral_account_id, "?"), None
...
def dashboard(db: Session, user: User) -> dict:
    ...
    return {
        "kpis": {
            ...
            "stale_count": sum(1 for o in all_open if is_stale(o)),
            "new_leads": new_leads,
