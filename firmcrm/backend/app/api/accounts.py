...
from app.api.common import SortDir, apply_sort, apply_updates, archive, get_or_404, paginate, restore, user_names
from app.core.audit import record
from app.core.db import get_db
from app.core.deps import at_least, get_current_user, require_role
from app.core.errors import DomainError
from app.enums import AccountType
from app.models import Account, Activity, Contact, Engagement, Opportunity, User
from app.schemas import AccountCreate, AccountOut, AccountUpdate, Page
from app.services import visibility

router = APIRouter(prefix="/accounts", tags=["accounts"])


def _enrich(db: Session, rows: list[Account]) -> list[AccountOut]:
...
def list_accounts(q: str | None = Query(None, max_length=200), account_type: AccountType | None = None, owner_id: int | None = None,
...
@router.get("/duplicates", response_model=list[AccountOut])
...
def create_account(body: AccountCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
def update_account(account_id: int, body: AccountUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
