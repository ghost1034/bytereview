...
from app.api.common import account_names, get_or_404, opportunity_names, paginate, user_names
from app.core.audit import record
from app.core.db import get_db
from app.core.deps import at_least, get_current_user
from app.core.errors import DomainError, NotFound
from app.enums import CheckType
from app.models import ConflictCheck, Opportunity, User, utcnow
from app.schemas import ConflictCheckCreate, ConflictCheckOut, ConflictMatch, ConflictResolveIn, ConflictSearchIn, Page
from app.services import conflicts as svc
from app.services import visibility

CheckStatus = Literal["pending", "clear", "conflict", "waived"]
router = APIRouter(prefix="/conflict-checks", tags=["conflict-checks"])


def _enrich(db: Session, rows: list[ConflictCheck]) -> list[ConflictCheckOut]:
    ...
    out = []
    for c in rows:
        d = ConflictCheckOut.model_validate(c)
...
@router.post("/search", response_model=list[ConflictMatch])
def search(body: ConflictSearchIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.get("", response_model=Page[ConflictCheckOut])
...
@router.post("", response_model=ConflictCheckOut, status_code=201)
def run_check(body: ConflictCheckCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
    ...
    if body.check_type == "independence" and body.independence_attestation:
        if any(bool(v) for v in body.independence_attestation.values()):
...
@router.get("/{check_id}", response_model=ConflictCheckOut)
...
@router.post("/{check_id}/resolve", response_model=ConflictCheckOut)
def resolve(check_id: int, body: ConflictResolveIn, db: Session = Depends(get_db), actor: User = Depends(at_least("manager"))):
