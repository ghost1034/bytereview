...
from app.models import Account, Opportunity, Pipeline, PracticeArea, Stage, StageHistory, User, utcnow
from app.schemas import OpportunityCreate, OpportunityOut, OpportunityUpdate, Page, StageChangeIn, StageHistoryOut
from app.services import conflicts, visibility
from app.services import opportunities as opp_svc
from app.services.leads import default_pipeline

router = APIRouter(prefix="/opportunities", tags=["opportunities"])


def enrich(db: Session, rows: list[Opportunity]) -> list[OpportunityOut]:
    ...
    out = []
    for o in rows:
        d = OpportunityOut.model_validate(o)
        ...
        d.is_stale = opp_svc.is_stale(o)
        d.clearance_type = pa_clear.get(o.practice_area_id)
        d.clearance_status = conflicts.latest_status_for_opportunity(db, o.id) if d.clearance_type else None
        out.append(d)
    return out
...
def list_opportunities(q: str | None = Query(None, max_length=200), status: str | None = Query("open", max_length=10),
                       pipeline_id: int | None = None, stage_id: int | None = None, owner_id: int | None = None,
    ...
    if stale_only:
        ...
        return Page(items=out[offset:offset + limit], total=len(out), limit=limit, offset=offset)
    rows, total = paginate(db, stmt, limit, offset)
    return Page(items=enrich(db, rows), total=total, limit=limit, offset=offset)


@router.post("", response_model=OpportunityOut, status_code=201)
def create_opportunity(body: OpportunityCreate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
def update_opportunity(opp_id: int, body: OpportunityUpdate, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
...
def change_stage(opp_id: int, body: StageChangeIn, db: Session = Depends(get_db), actor: User = Depends(get_current_user)):
    o = get_visible(db, actor, opp_id)
...
@router.post("/{opp_id}/reopen", response_model=OpportunityOut)
def reopen(opp_id: int, body: StageChangeIn, db: Session = Depends(get_db), actor: User = Depends(at_least("manager"))):
    o = get_visible(db, actor, opp_id)
...
@router.get("/{opp_id}/history", response_model=list[StageHistoryOut])
def history(opp_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    out = []
    for h in rows:
        d = StageHistoryOut.model_validate(h)
...
@router.post("/{opp_id}/archive", response_model=OpportunityOut)
...
@router.post("/{opp_id}/restore", response_model=OpportunityOut)
