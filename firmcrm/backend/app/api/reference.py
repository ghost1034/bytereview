...
from app.models import Opportunity, Pipeline, PracticeArea, Stage, User
from app.schemas import PipelineIn, PipelineOut, PracticeAreaIn, PracticeAreaOut, StageIn, StageOut

router = APIRouter(tags=["reference"])


@router.get("/practice-areas", response_model=list[PracticeAreaOut])
...
@router.post("/practice-areas", response_model=PracticeAreaOut, status_code=201)
def create_practice_area(body: PracticeAreaIn, db: Session = Depends(get_db), actor: User = Depends(require_role("admin"))):
...
@router.patch("/practice-areas/{pa_id}", response_model=PracticeAreaOut)
def update_practice_area(pa_id: int, body: PracticeAreaIn, db: Session = Depends(get_db), actor: User = Depends(require_role("admin"))):
...
def create_pipeline(body: PipelineIn, db: Session = Depends(get_db), actor: User = Depends(require_role("admin"))):
...
@router.post("/pipelines/{pipeline_id}/stages", response_model=StageOut, status_code=201)
def add_stage(pipeline_id: int, body: StageIn, db: Session = Depends(get_db), actor: User = Depends(require_role("admin"))):
...
def update_stage(stage_id: int, body: StageIn, db: Session = Depends(get_db), actor: User = Depends(require_role("admin"))):
