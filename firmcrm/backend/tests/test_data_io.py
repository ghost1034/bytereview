...
def test_export_requires_manager_and_streams_csv(client, admin):
    ...
    assert client.get("/api/export/accounts.csv", headers=staff).status_code == 403
    r = client.get("/api/export/accounts.csv", headers=admin)
    assert r.status_code == 200 and r.headers["content-type"].startswith("text/csv")
    rows = list(csv.DictReader(io.StringIO(r.text)))
    assert len(rows) >= 24 and "name" in rows[0] and "owner" in rows[0]
    assert client.get("/api/export/unicorns.csv", headers=admin).status_code == 422
    audit = client.get("/api/admin/audit?entity_type=export", headers=admin).json()["items"]
    assert audit and audit[0]["action"] == "export.accounts"
    # opportunities export includes computed weighted column
    r = client.get("/api/export/opportunities.csv", headers=admin)
    row = next(csv.DictReader(io.StringIO(r.text)))
    assert "weighted" in row and "stage" in row


def test_import_template(client, admin):
    r = client.get("/api/import/template/contacts.csv", headers=admin)
...
def test_import_dry_run_reports_exceptions_and_writes_nothing(client, admin):
    before = client.get("/api/accounts?limit=1", headers=admin).json()["total"]
    rows = [
    ...
    r = client.post("/api/import/accounts", files={"file": ("accounts.csv", data, "text/csv")}, data={"dry_run": "true"}, headers=admin)
    ...
    # Nothing persisted
    assert client.get("/api/accounts?limit=1", headers=admin).json()["total"] == before
    assert client.get("/api/accounts?q=Imported%20Co", headers=admin).json()["total"] == 0
    # Exceptions report downloadable
    rep = client.get(f"/api/import/jobs/{job['id']}/exceptions.csv", headers=admin)
...
def test_import_commit_is_idempotent_upsert(client, admin):
    ...
    r = client.post("/api/import/contacts", files={"file": ("c.csv", _csv(rows, headers), "text/csv")}, data={"dry_run": "false"}, headers=admin)
    assert r.status_code == 200, r.text
    job = r.json()
    assert job["created_rows"] == 1 and job["skipped_rows"] == 1
    c = client.get("/api/contacts?q=imp.orted", headers=admin).json()["items"][0]
    ...
    assert client.get("/api/contacts?q=imp.orted", headers=admin).json()["total"] == 1
    assert client.get("/api/contacts?q=imp.orted", headers=admin).json()["items"][0]["title"] == "Chief Financial Officer"
    jobs = client.get("/api/import/jobs", headers=admin).json()
    assert jobs["total"] >= 3
    audit = client.get("/api/admin/audit?action=import.", headers=admin).json()["items"]
    assert len(audit) >= 2
...
def test_import_leads_with_practice_area(client, admin):
    ...
    r = client.post("/api/import/leads", files={"file": ("l.csv", _csv(rows, headers), "text/csv")}, data={"dry_run": "false"}, headers=admin)
    job = r.json()
    assert job["created_rows"] == 1 and job["skipped_rows"] == 1
    fields = {e["field"] for e in job["exceptions"]}
    assert "practice_area" in fields
    lead = client.get("/api/leads?q=importleads", headers=admin).json()["items"][0]
...
def test_import_rejects_bad_files(client, admin):
    r = client.post("/api/import/accounts", files={"file": ("x.csv", b"", "text/csv")}, data={"dry_run": "true"}, headers=admin)
    assert r.status_code == 400
    r = client.post("/api/import/accounts", files={"file": ("x.csv", b"foo,bar\n1,2\n", "text/csv")}, data={"dry_run": "true"}, headers=admin)
