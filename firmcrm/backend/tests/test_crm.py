...
def test_health_and_auth(client):
    ...
    r = client.post("/api/auth/login", json={"email": "admin@demo.firm", "password": "wrong"})
    assert r.status_code == 401


def test_lead_convert_creates_account_contact_opportunity(client, admin):
    r = client.post("/api/leads", json={"first_name": "Ada", "last_name": "Lovelace", "company": "Analytical Engines Ltd",
                                         "email": "ada@ae.example.com", "source": "referral", "estimated_value": 50000}, headers=admin)
    assert r.status_code == 201
    lead_id = r.json()["id"]
    r = client.post(f"/api/leads/{lead_id}/convert", json={"create_opportunity": True}, headers=admin)
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["account_id"] and body["contact_id"] and body["opportunity_id"]
    acc = client.get(f"/api/accounts/{body['account_id']}", headers=admin).json()
    assert acc["name"] == "Analytical Engines Ltd" and acc["contact_count"] == 1
    opp = client.get(f"/api/opportunities/{body['opportunity_id']}", headers=admin).json()
    assert opp["amount"] == 50000 and opp["stage_name"] == "Identified"
    # second convert is rejected
    assert client.post(f"/api/leads/{lead_id}/convert", json={}, headers=admin).status_code == 400


def test_won_requires_clearance_and_engagement_letter(client, admin):
    pas = client.get("/api/practice-areas", headers=admin).json()
    ...
    acc = client.post("/api/accounts", json={"name": "Gatekeeper Test Co"}, headers=admin).json()
    opp = client.post("/api/opportunities", json={"name": "Gatekeeper – litigation", "account_id": acc["id"],
    ...
    r = client.post(f"/api/opportunities/{opp['id']}/stage", json={"stage_id": stages["Closed Won"]}, headers=admin)
    ...
    r = client.post("/api/conflict-checks", json={"opportunity_id": opp["id"], "parties": ["Gatekeeper Test Co", "Northwind Robotics"]}, headers=admin)
    assert r.status_code == 201
    chk = r.json()
    ...
    assert client.post(f"/api/conflict-checks/{chk['id']}/resolve", json={"status": "waived"}, headers=staff).status_code == 403
    partner = login(client, "partner.lit@demo.firm")
    assert client.post(f"/api/conflict-checks/{chk['id']}/resolve", json={"status": "waived"}, headers=partner).status_code == 400
    r = client.post(f"/api/conflict-checks/{chk['id']}/resolve", json={"status": "waived", "resolution_note": "Informed consent obtained from Northwind"}, headers=partner)
    ...
    client.patch(f"/api/opportunities/{opp['id']}", json={"engagement_letter_status": "signed"}, headers=admin)
    r = client.post(f"/api/opportunities/{opp['id']}/stage", json={"stage_id": stages["Closed Won"]}, headers=admin)
    assert r.status_code == 200, r.text
    ...
    engs = client.get(f"/api/engagements?account_id={acc['id']}", headers=admin).json()["items"]
    assert len(engs) == 1 and engs[0]["annual_value"] == 100000
    hist = client.get(f"/api/opportunities/{opp['id']}/history", headers=admin).json()
...
def test_lost_requires_reason(client, admin):
    ...
    r = client.post(f"/api/opportunities/{opp['id']}/stage", json={"stage_id": lost}, headers=admin)
    assert r.status_code == 400 and r.json()["code"] == "lost_reason"


def test_conflict_search_fuzzy(client, admin):
    r = client.post("/api/conflict-checks/search", json={"parties": ["Brightline Transport", "brightline logistics"]}, headers=admin)
...
def test_activity_touches_opportunity(client, admin):
    opp = client.get("/api/opportunities?status=open", headers=admin).json()["items"][0]
    r = client.post("/api/activities", json={"kind": "call", "subject": "Check-in", "opportunity_id": opp["id"]}, headers=admin)
    assert r.status_code == 201 and r.json()["account_id"] == opp["account_id"]
    assert client.get(f"/api/opportunities/{opp['id']}", headers=admin).json()["is_stale"] is False
...
def test_reports(client, admin):
    for path in ("dashboard", "pipeline", "win-loss", "practice-areas", "origination", "referral-sources", "funnel",
        ...
        r = client.get(f"/api/reports/{path}", headers=admin)
        assert r.status_code == 200, path
    d = client.get("/api/reports/dashboard", headers=admin).json()
    assert d["kpis"]["open_count"] > 0 and d["kpis"]["pending_clearances"] >= 2


def test_audit_trail(client, admin):
    rows = client.get("/api/admin/audit?entity_type=opportunity", headers=admin).json()["items"]
    assert any(r["action"] == "opportunity.stage_change" for r in rows)
    staff = login(client, "staff@demo.firm")
    assert client.get("/api/admin/audit", headers=staff).status_code == 403
