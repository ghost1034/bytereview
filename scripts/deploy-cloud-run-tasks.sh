...
# Shared worker modules construct CloudRunTaskService during import, so every
# task container needs the complete service URL configuration at startup.
TASK_SERVICE_URL_SECRETS="TASK_EXTRACT_URL=TASK_EXTRACT_URL:latest,TASK_IO_URL=TASK_IO_URL:latest,TASK_AUTOMATION_URL=TASK_AUTOMATION_URL:latest,TASK_MAINTENANCE_URL=TASK_MAINTENANCE_URL:latest"
...
deploy_service \
    ...
    "--add-cloudsql-instances=$CLOUD_SQL_INSTANCE \
     ...
     --no-cpu-throttling \
     --set-secrets=$TASK_SERVICE_URL_SECRETS,DATABASE_URL=DATABASE_URL:latest,GEMINI_API_KEY=GEMINI_API_KEY:latest,STRIPE_SECRET_KEY=STRIPE_SECRET_KEY:latest,ENCRYPTION_KEY=ENCRYPTION_KEY:latest,/var/secrets/google/service-account.json=FIREBASE_SERVICE_ACCOUNT:latest \
...
deploy_service \
    ...
    "--add-cloudsql-instances=$CLOUD_SQL_INSTANCE \
     ...
     --service-account=$SERVICE_ACCOUNT \
     --no-cpu-throttling \
     --set-secrets=$TASK_SERVICE_URL_SECRETS,DATABASE_URL=DATABASE_URL:latest,GOOGLE_CLIENT_ID=GOOGLE_CLIENT_ID:latest,GOOGLE_CLIENT_SECRET=GOOGLE_CLIENT_SECRET:latest,GOOGLE_REDIRECT_URI=GOOGLE_REDIRECT_URI:latest,STRIPE_SECRET_KEY=STRIPE_SECRET_KEY:latest,ENCRYPTION_KEY=ENCRYPTION_KEY:latest,ESIGN_SIGNING_CERT_PEM=$ESIGN_SIGNING_CERT_SECRET:latest,/var/secrets/google/service-account.json=FIREBASE_SERVICE_ACCOUNT:latest \
