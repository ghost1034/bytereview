...
usage() {
    ...
    echo "Options:"
    echo "  --frontend-only  Build/deploy only the frontend service"
    echo "  --backend-only   Build/deploy only the API and task services"
...
set_target() {
    if [ "$DEPLOY_TARGET" != "all" ] && [ "$DEPLOY_TARGET" != "$1" ]; then
...
if [ "$RUN_DEPLOY" = true ]; then
    if [ "$DEPLOY_TARGET" = "all" ]; then
        ...
        print_section "Deploying Backend Service (API)"
    else
    ...
    if [ "$DEPLOY_TARGET" = "frontend" ]; then
        DEPLOY_SERVICE_ARGS+=(--frontend-only)
    elif [ "$DEPLOY_TARGET" = "backend" ]; then
        DEPLOY_SERVICE_ARGS+=(--backend-only)
    fi
...
if [ "$RUN_DEPLOY" = false ]; then
    ...
    echo -e "${GREEN}✅ ${TARGET_LABEL} images were built and pushed successfully!${NC}"
    TARGET_ARG=""
    if [ "$DEPLOY_TARGET" = "frontend" ]; then
    ...
    if [ "$ENVIRONMENT" = "staging" ]; then
        ...
        echo -e "${YELLOW}Deploy them with: ./scripts/deploy.sh --deploy-only${TARGET_ARG}${NC}"
    fi
    exit 0
fi

print_section "Deployment Complete!"
echo -e "${GREEN}✅ CPAAutomation ${DEPLOY_TARGET} deployment completed successfully!${NC}"
echo ""
