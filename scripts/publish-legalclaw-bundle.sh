#!/usr/bin/env bash
# Publishes the PLAINTEXT LegalClaw profile bundle for desktop installs.
#
# Builds the same tarball the Docker image's bundle stage builds
# (tar -C hermes/legalclaw/profile .) and uploads it, unencrypted, to the
...
# inside the image (see scripts/build-legalclaw-image.sh).

set -euo pipefail

BUCKET="${CPAA_BUNDLE_GCS_BUCKET:-cpaa-accountingclaw-bundles}"
OBJECT="${CPAA_LEGALCLAW_BUNDLE_GCS_OBJECT:-legalclaw/legalclaw-profile.tar.gz}"
PROFILE_DIR="${LEGALCLAW_PROFILE_DIR:-hermes/legalclaw/profile}"
...
tar -C "$PROFILE_DIR" -czf "$tmp_dir/legalclaw-profile.tar.gz" .

if command -v sha256sum >/dev/null 2>&1; then
  SHA="$(sha256sum "$tmp_dir/legalclaw-profile.tar.gz" | awk '{print $1}')"
else
  SHA="$(shasum -a 256 "$tmp_dir/legalclaw-profile.tar.gz" | awk '{print $1}')"
fi
...
gsutil "${headers[@]}" cp "$tmp_dir/legalclaw-profile.tar.gz" "gs://$BUCKET/$OBJECT"
