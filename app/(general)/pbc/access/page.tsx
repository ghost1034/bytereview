...
function Portal() {
  ...
  const selectedDependencies = selected?.dependency_ids.map((id) => workspace?.requests.find((item) => item.id === id)).filter(Boolean) as PbcRequestItem[] | undefined
      ...
      <div className="grid min-h-[600px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          {selected && <div className="flex h-full flex-col">
            <div className="border-b border-slate-200 p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-slate-500">{selected.request_number} · {selected.category || 'Other'} · {label(selected.priority)} priority</p><h2 className="mt-1 text-xl font-semibold text-slate-950">{selected.title}</h2></div><Badge className={statusTone[selected.status]}>{label(selected.status)}</Badge></div>{selected.description && <p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-slate-600">{selected.description}</p>}<dl className="mt-4 grid gap-3 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2"><div><dt className="text-xs text-slate-500">Expected filename</dt><dd className="mt-0.5 text-slate-900">{selected.expected_filename || 'Any filename'}</dd></div><div><dt className="text-xs text-slate-500">Expected format</dt><dd className="mt-0.5 text-slate-900">{selected.expected_formats.length ? selected.expected_formats.map((format) => `.${format}`).join(', ') : 'Any permitted format'}</dd></div>{selected.period_end && <div><dt className="text-xs text-slate-500">Period end</dt><dd className="mt-0.5 text-slate-900">{selected.period_end}</dd></div>}{selected.gl_account && <div><dt className="text-xs text-slate-500">GL reference</dt><dd className="mt-0.5 text-slate-900">{selected.gl_account}{selected.gl_balance ? ` · ${selected.gl_balance}` : ''}</dd></div>}{selectedDependencies && selectedDependencies.length > 0 && <div className="sm:col-span-2"><dt className="text-xs text-slate-500">Complete first</dt><dd className="mt-0.5 text-slate-900">{selectedDependencies.map((item) => `${item.request_number} ${item.title}`).join(' · ')}</dd></div>}{selected.requires_redaction && <div className="sm:col-span-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-900">Remove protected or unnecessary personal information before uploading.</div>}</dl>{selected.status_reason && <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800"><strong>Reviewer requested changes:</strong> {selected.status_reason}</div>}</div>
            <div className="grid flex-1 gap-6 p-6 xl:grid-cols-2">
              <section><div className="flex items-center justify-between"><div><h3 className="font-medium text-slate-950">Evidence</h3><p className="text-xs text-slate-500">Files are versioned.</p></div>{['open', 'needs_changes'].includes(selected.status) && <Button size="sm" onClick={() => inputRef.current?.click()} disabled={busy === 'upload'}><Upload className="mr-2 size-4" />{busy === 'upload' ? 'Uploading…' : 'Upload'}</Button>}</div><input ref={inputRef} className="hidden" type="file" accept={selected.expected_formats.length ? selected.expected_formats.map((format) => `.${format}`).join(',') : undefined} onChange={(event) => upload(event.target.files?.[0])} /><div className="mt-4 space-y-2">{selected.documents.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center"><File className="mx-auto size-7 text-slate-400" /><p className="mt-2 text-sm text-slate-500">No files uploaded yet.</p></div>}{selected.documents.map((document) => <button key={document.id} onClick={() => download(document.id)} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 p-3 text-left hover:bg-slate-50"><File className="size-4 text-blue-600" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-slate-900">{document.filename}</span><span className="text-xs text-slate-500">Version {document.version}</span></span><Download className="size-4 text-slate-400" /></button>)}</div>{['open', 'needs_changes'].includes(selected.status) && <Button className="mt-4 w-full" onClick={submit} disabled={busy === 'submit' || selected.documents.filter((item) => item.state === 'available').length === 0}><CheckCircle2 className="mr-2 size-4" />{busy === 'submit' ? 'Submitting…' : 'Submit for review'}</Button>}{selected.status === 'submitted' && <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">Submitted for firm review. You will be notified if changes are needed.</div>}{selected.status === 'accepted' && <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-800">Accepted by the firm. No further action is needed.</div>}</section>
