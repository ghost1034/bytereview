...
function SettingsPanel() {
  ...
  React.useEffect(() => { if (settings.data) { setDays(settings.data.expiration_days == null ? '' : String(settings.data.expiration_days)); setReminderHours(settings.data.reminder_interval_hours == null ? '' : String(settings.data.reminder_interval_hours)); setBrandId(settings.data.default_brand_id == null ? '' : String(settings.data.default_brand_id)) } }, [settings.data])
  return <div className="space-y-5"><div className="flex items-center gap-3"><Settings2 className="size-5 text-primary" /><div><h2 className="text-lg font-semibold">Sending defaults & features</h2><p className="text-sm text-foreground-muted">Locked values are revalidated at send time.</p></div></div><div className="max-w-md space-y-4 rounded-lg border border-border p-4"><label className="block text-sm font-medium">Default expiration (days)<Input className="mt-2" type="number" min={1} max={3650} value={days} onChange={(event) => setDays(event.target.value)} placeholder="No default expiration" /></label><label className="block text-sm font-medium">Default reminder interval (hours)<Input className="mt-2" type="number" min={1} max={720} value={reminderHours} onChange={(event) => setReminderHours(event.target.value)} placeholder="No automatic reminders" /></label><label className="block text-sm font-medium">Default brand<select className="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={brandId} onChange={(event) => setBrandId(event.target.value)}><option value="">CPAAutomation default (no firm brand)</option>{brands.data?.brands.filter((brand) => brand.active).map((brand) => <option key={String(brand.id)} value={String(brand.id)}>{String(brand.name)}</option>)}</select></label><div className="grid grid-cols-2 gap-2 text-xs text-foreground-muted">{Object.entries(settings.data?.features ?? {}).map(([name, enabled]) => <div key={name} className="rounded bg-surface-muted px-2 py-1.5">{enabled ? 'On' : 'Off'} · {name.replace(/_/g, ' ')}</div>)}</div><Button onClick={async () => { try { await apiClient.updateEsignAdminSettings({ expiration_days: days ? Number(days) : null, reminder_interval_hours: reminderHours ? Number(reminderHours) : null, default_brand_id: brandId || null }); await queryClient.invalidateQueries({ queryKey: ['esign', 'admin', 'settings'] }); toast({ title: 'Settings saved' }) } catch (error) { toast({ title: 'Could not save settings', description: error instanceof Error ? error.message : undefined, variant: 'destructive' }) } }}>Save defaults</Button></div></div>
}
