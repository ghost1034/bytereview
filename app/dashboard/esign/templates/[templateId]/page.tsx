...
export default function EsignTemplateEditPage() {
  ...
  return (
    <ComposerShell title={template.name} stage="fields" saveState={saveState} onClose={() => router.push('/dashboard/esign/templates')} primary={<div className="flex gap-2"><Button variant="outline" onClick={() => setConfigOpen(value => !value)}><Settings2 className="mr-1.5 size-4" /> Settings</Button><Button variant="outline" disabled={publishTemplate.isPending || !!template.archived_at || aiPlacementPending || formulaSetupPending} onClick={async () => { try { if (aiPlacementPending || formulaSetupPending) { toast({ title: formulaSetupPending ? 'Finish formula setup first' : 'Review AI field suggestions first', description: formulaSetupPending ? 'Apply or cancel the staged formula before publishing.' : 'Apply or discard the staged suggestions before publishing.' }); return } if (!await handleSave()) return; const version = await publishTemplate.mutateAsync({ templateId: template.id, expectedRevision: draftRevision.current }); toast({ title: `Version ${version.version} published`, description: 'Bulk jobs and PowerForms can now pin this immutable version.' }) } catch (error) { toast({ title: 'Publish failed', description: error instanceof Error ? error.message : undefined, variant: 'destructive' }) } }}><UploadCloud className="mr-1.5 size-4" /> Publish</Button><Button disabled={!!template.archived_at || aiPlacementPending || formulaSetupPending} onClick={async () => { if (await handleSave()) router.push(`/dashboard/esign/new?template=${template.id}`) }}><Send className="mr-1.5 size-4" /> Use template</Button></div>}>
