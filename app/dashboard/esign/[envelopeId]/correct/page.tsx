...
export default function CorrectRecipientsPage() {
  ...
  const save = async () => {
    if (formulaSetupPending) { toast({ title: 'Finish or cancel formula setup first', variant: 'destructive' }); return }
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="space-y-3 rounded-lg border border-border bg-surface p-5"><p className="text-sm font-medium">Change summary: {changed} recipient change{changed === 1 ? '' : 's'}{fieldsChanged ? ' · field layout changed' : ''}</p>{validationError && <p className="text-sm text-warning">{validationError}</p>}{formulaSetupPending && <p className="text-sm text-warning">Finish or cancel the staged formula before committing this correction.</p>}<div><Label htmlFor="correction-reason">Required reason</Label><Textarea id="correction-reason" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explain why recipient routing, documents, or fields are being corrected…" /></div><Button disabled={saving || formulaSetupPending || (changed === 0 && !fieldsChanged) || !reason.trim() || !!validationError} onClick={() => void save()}>{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Save className="mr-2 size-4" />} Commit correction</Button></section>
  </div>
}
