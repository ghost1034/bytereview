  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="space-y-3 rounded-lg border border-border bg-surface p-5">{rows.map((row, index) => {
      ...
      return <div key={row.id ?? index} className="grid gap-3 rounded-lg border border-border p-3 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div><Label className="text-xs">Role</Label><Select disabled={locked} value={row.role} onValueChange={(role) => update({ role: role as typeof row.role })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{allowedRoles.map((role) => <SelectItem key={role} value={role}>{role.replace(/_/g, ' ')}</SelectItem>)}</SelectContent></Select></div>
        <div className="flex items-end gap-2"><div className="flex-1"><Label className="text-xs">Step</Label><Input disabled={locked} type="number" min={1} value={row.routing_order} onChange={(event) => update({ routing_order: Math.max(1, Number(event.target.value) || 1) })} /></div><Button variant="ghost" size="icon" disabled={locked} onClick={() => setRows((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="size-4" /></Button></div>
        ...
        {['signer', 'approver', 'certified_delivery'].includes(row.role) && <div><Label className="text-xs">Managed by</Label><Select disabled={locked} value={row.managed_by_recipient_id ?? 'none'} onValueChange={(value) => update({ managed_by_recipient_id: value === 'none' ? null : value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Not managed</SelectItem>{rows.filter((item) => item.id && ['agent', 'editor'].includes(item.role)).map((manager) => <SelectItem key={manager.id} value={manager.id!}>{manager.name || manager.role_label || manager.role}</SelectItem>)}</SelectContent></Select></div>}
        ...
        <label className="flex items-center gap-2 self-end text-sm"><input disabled={locked} type="checkbox" checked={row.allow_reassignment ?? false} onChange={(event) => update({ allow_reassignment: event.target.checked })} /> Allow reassignment</label>
      </div>
